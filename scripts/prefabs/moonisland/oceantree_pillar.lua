...
local assets =
{
    Asset("ANIM", "anim/moonisland/oceantree_pillar_small_build1.zip"),
    Asset("ANIM", "anim/moonisland/oceantree_pillar_small_build2.zip"),
    Asset("ANIM", "anim/moonisland/oceantree_pillar_small.zip"),
...
local MIN = TUNING.SHADE_CANOPY_RANGE_SMALL or 22
local MAX = MIN + (TUNING.WATERTREE_PILLAR_CANOPY_BUFFER or 1)
    ...
    if boat_physics ~= nil then
        ...
        if hit_velocity > 0.8 then
            inst:DoTaskInTime(0, function()                
                ...
                local time = clock ~= nil and clock:GetTotalTime() or 0
...
local function fn()
    ...
    MakeObstaclePhysics(inst, 1.5, 2, 0.75) --MakeWaterObstaclePhysics(inst, 1.5, 2, 0.75)

    --inst:SetDeployExtraSpacing(TUNING.MAX_WALKABLE_PLATFORM_RADIUS + 1.5) --改，暂时禁用
    ...
    inst.MiniMapEntity:SetIcon("oceantree_pillar_small.tex")
...
--[[
    ...
    inst.components.playerprox:SetDist(MIN+7, MAX+7)
...
--]]
    ...
    inst.pool = SpawnPrefab("oceantree_pool")
    inst.pool.entity:SetParent(inst.entity)
...
local function leaves_fn(data)
    ...
    inst:DoTaskInTime(0, function()  
            ...
            if not inst:IsOnValidGround() then --if not TheWorld.Map:IsVisualGroundAtPoint(point.x,point.y,point.z) then
...
return Prefab("oceantree_pillar", fn, assets, prefabs),
    ...
    Prefab("oceantree_pool", pool_fn, assets),
	   MakePlacer( "common/oceantree_pillar_placer", "pinecone", "pinecone", "idle_planted" )
