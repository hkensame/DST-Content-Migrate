local moon_tree_assets = {
    Asset("ANIM", "anim/moonisland/moon_tree.zip"),
...
local function on_moon_tree_burnt_immediate_helper(inst, immediate)
    ...
    inst.MiniMapEntity:SetIcon("moon_tree_burnt.tex")
...
local function make_stump(inst)
   ...
   -- MakeMediumPropagator(inst)
    ...
    inst.MiniMapEntity:SetIcon("moon_tree_stump.tex")
...
local function on_chop_tree_down(inst, chopper)
    ...
    --inst:DoTaskInTime(0.4, function (inst)
        --改ShakeAllCameras( CAMERASHAKE.FULL, .25, .03, (inst.size == TALL and .5) or .25, inst, 6 )
    --end)
...
local function moon_tree(name, stage, data)
    local function fn()
        ...
        inst.MiniMapEntity:SetIcon("moon_tree.tex")
