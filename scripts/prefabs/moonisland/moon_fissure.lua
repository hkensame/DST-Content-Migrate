...
local assets =
{
    Asset("ANIM", "anim/moonisland/moon_fissure.zip"),
    Asset("ANIM", "anim/moonisland/moon_fissure_fx.zip"),
}

local assets_plugged =
{
    Asset("ANIM", "anim/moonisland/plugged_fissure.zip"),
}
...
local function OnUpdateLight(inst)
	local lightstate = lightstate_data[inst._level] or lightstate_data[1]
	local world_brightness = 1 --GetWorld().components.ambientlighting:GetVisualAmbientValue()
...
local function UpdateState(inst)    
    local moonphase = GetWorld().components.clock:GetMoonPhase()
	local level = MOON_STATES[moonphase] or 0
...
local function aurafn(inst, observer)
	return (lightstate_data[inst._level] or lightstate_data[1]).sanityaura
end
...
local function getstatus(inst)
    return (inst._level == 1 and "NOLIGHT") or
           nil
end
...
local function fn()
    ...
    --改inst:SetPhysicsRadiusOverride(0.4)
    ...
    inst._level = 0 --net_tinybyte(inst.GUID, "moonfissure.level", "leveldirty")
    
    local moonphase = GetWorld().components.clock:GetMoonPhase()
    inst._level = (MOON_STATES[moonphase])
    ...
    inst:AddComponent("repairable") --被修复的对象
    inst.components.repairable.repairmaterial = "MOON_ALTAR"
    inst.components.repairable.onrepaired = on_piece_slotted
--[[
    inst.components.repairable.onrepaired = function()--on_piece_slotted
        GetPlayer().components.talker:Say("修 修 修！就知道修！不知道bug很难修吗！")
    end--]]
    ...
    --改，暂时禁用inst:WatchWorldState("moonphase", onmoonphasechagned)
...
local function plugged_fn()
    ...
    inst:ListenForEvent("animover", function()
        if inst.AnimState:IsCurrentAnimation("toot_pre") then
            ...
            GetWorld():PushEvent("moonfissurevent",inst)
        end
    end)
