...
local guard_assets =
{
    Asset("ANIM", "anim/alterguardian/brightmare_gestalt_evolved.zip"),
}
local guard_prefabs =
{
    "gestalt_guard_head",
...
local function attack_behaviour(inst, target)
    if inst.components.combat ~= nil then
        ...
        if target.components.grogginess_dst ~= nil and not target.sg:HasStateTag("knockout") then
            target.components.grogginess_dst:AddGrogginess(TUNING.GESTALT_ATTACK_DAMAGE_GROGGINESS, TUNING.GESTALT_ATTACK_DAMAGE_KO_TIME)
            if target.components.grogginess_dst.knockoutduration == 0 then
...
local DEFAULT_FINDVICTIM_RANGE = math.sqrt(TUNING.GESTALT_ATTACK_HIT_RANGE_SQ or 64)
...
local HATGUARD_COMBAT_CANTHAVE_TAGS = { "INLIMBO", "structure", "wall", "player" }

local function hatguard_find_attack_victim(inst)
    local x, y, z = inst.Transform:GetWorldPosition()
	local ents = TheSim:FindEntities(x, y, z, 0.75, nil, HATGUARD_COMBAT_CANTHAVE_TAGS)
	for _, target in ipairs(ents) do
		if (target.components.health ~= nil and not target.components.health:IsDead())
			and (target.components.combat ~= nil and inst.components.combat:CanTarget(target)) then
			return target
		end
	end
end
...
local function hatguardfn()
    ...
    inst:AddComponent("combat")
    inst.components.combat:SetDefaultDamage(TUNING.ALTERGUARDIANHAT_GESTALT_DAMAGE or 42.5)
    inst.components.combat:SetRange(TUNING.GESTALTGUARD_ATTACK_RANGE)
