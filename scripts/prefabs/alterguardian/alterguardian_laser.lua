...
local function DoDamage(inst, targets, skiptoss, skipscorch)
    ...
    if inst.AnimState ~= nil then
        ...
        inst:Show()
        inst:DoTaskInTime(inst.AnimState:GetCurrentAnimationLength(inst) + 2 * FRAMES, inst.Remove)

        inst.Light:Enable(true)
        inst:DoTaskInTime(4 * FRAMES, SetLightRadius, .5)
        inst:DoTaskInTime(5 * FRAMES, DisableLight)

        if not skipscorch and IsPassableAtPoint(x, 0, z, false) then
    ...
    for _, v in ipairs(TheSim:FindEntities(x, 0, z, RADIUS + 3, nil, DAMAGE_CANT_TAGS)) do --, DAMAGE_ONEOF_TAGS)) do
            ...
            if dsq_to_laser < range * range then
                ...
                elseif inst.components.combat:CanTarget(v) then
                    ...
                    if inst.caster ~= nil and inst.caster:IsValid() then
                        ...
                        --inst.caster.components.combat:DoAttack(v) --原
                        v.components.combat:GetAttacked(inst.caster, 225) --改
    ...
    local platform_hit = GetPlatformAtPoint(x, 0, z)
...
local function Scorch_OnFadeDirty(inst)
    ...
    if not inst._fade then return end
...
local function scorchfn()
    ...
    inst:DoTaskInTime(2, inst.Remove) --圆弧激光痕迹消失
    inst.persists = false
...
local function FastForwardTrail(inst, pct)
    ...
    local len = inst.AnimState:GetCurrentAnimationLength(inst)
...
local function trailfn()
    ...
    inst.persists = false
    inst._task = inst:DoTaskInTime(inst.AnimState:GetCurrentAnimationLength(inst) + 2 * FRAMES, inst.Remove)
