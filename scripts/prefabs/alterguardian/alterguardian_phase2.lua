local assets =
{
    Asset("ANIM", "anim/alterguardian/alterguardian_phase2.zip"),
...
local SPIKE_RANGE = (TUNING.ALTERGUARDIAN_PHASE2_SPIKE_RANGE or 10)
local SPIKE_DSQ = SPIKE_RANGE * SPIKE_RANGE --100
...
local function do_spike_attack(inst)
    ...
    local p = inst --GetPlayer()
    --for k = 1, 3 do
        if not p:HasTag("playerghost") and p.entity:IsVisible()
                and (p.components.health ~= nil and not p.components.health:IsDead())
                and p:GetDistanceSqToPoint(ipos:Get()) < SPIKE_DSQ then --100
        --GetPlayer().components.talker:Say("距离："..p:GetDistanceSqToPoint(ipos:Get()))
...
local TARGET_DIST = TUNING.ALTERGUARDIAN_PHASE2_TARGET_DIST or 30
...
local PHASE2_MAXHEALTH = (TUNING.ALTERGUARDIAN_PHASE2_MAXHEALTH or 20000)
local PHASE2_STARTHEALTH = (TUNING.ALTERGUARDIAN_PHASE2_STARTHEALTH or 13000)
local HEALTH_GAIN_RATE = (PHASE2_MAXHEALTH - PHASE2_STARTHEALTH) / ((TUNING.TOTAL_DAY_TIME or 480) * 2)
...
local function fn()
    ...
    inst.DoSpikeAttack = do_spike_attack --改，为什么不执行
