...
local function emerge(inst)
    ...
    if WALL_SPIKE_COUNT > 1 then
        ...
        for wall_index = 2, WALL_SPIKE_COUNT do
            ...
            inst:DoTaskInTime(step*WALL_SPIKE_DELAY, function(inst2)
                ...
                if IsPassableAtPoint(spawn_point:Get()) then
    ...
    if player then
      player.components.playercontroller:ShakeCamera(inst, "FULL", .25, 0.05, 0.075, 45)
    end
...
local function check_over_water(inst)
    ...
    if not IsPassableAtPoint(check_pos:Get()) then
        emerge(inst)
...
local function MakeSpikeTrailPhysics(inst)
    ...
    phys:CollidesWith(COLLISION.WORLD or 192)
