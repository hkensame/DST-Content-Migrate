local assets =
{
        Asset("ANIM", "anim/alterguardian/alterguardian_meteor.zip"),
}
...
local function do_landed(inst)--改，空降晶石伤害
    ...
    if player then
      player.components.playercontroller:ShakeCamera(inst, "VERTICAL", .5, 0.1, 0.1, 35)
    end
...
local function do_groggy_pulse(inst)
    ...
    local nearby_targets = TheSim:FindEntities(
        ...
        nil, --[[PULSE_MUST_TAGS,]] PULSE_CANT_TAGS
    )
    ...
    for _, target in ipairs(nearby_targets) do
        if target.entity:IsVisible()
            ...
            if target.components.grogginess_dst ~= nil and not target.sg:HasStateTag("knockout") then
                target.components.grogginess_dst:AddGrogginess(TUNING.ALTERGUARDIAN_PHASE3_TRAP_GROGGINESS, TUNING.ALTERGUARDIAN_PHASE3_TRAP_KNOCKOUTTIME)
...
local function trap_fn()
    ...
    --inst:ListenForEvent("onalterguardianlasered", spawn_gestalt) --改，暂时禁用
