local assets =
{
    Asset("ANIM", "anim/alterguardian/alterguardian_phase3.zip"),
    Asset("ANIM", "anim/alterguardian/alterguardian_spawn_death.zip"),
}
...
--MUSIC------------------------------------------------------------------------
--]]
local PHASE3_TARGET_DIST = (TUNING.ALTERGUARDIAN_PHASE3_TARGET_DIST or 20)
local TARGET_DSQ = (1.9*PHASE3_TARGET_DIST)^2
...
local function Retarget(inst)
    ...
    if spawnpoint_position ~= nil and
        ...
        return FindEntity(
            inst,
            TUNING.ALTERGUARDIAN_PHASE3_TARGET_DIST or 20,
            function(guy)
...
local PHASE3_ATTACK_RANGE = (TUNING.ALTERGUARDIAN_PHASE3_ATTACK_RANGE or 14)
local MAX_KEEPTARGET_DSQ = 1.15 * PHASE3_ATTACK_RANGE
...
local function SpawnTrapProjectile(inst, target_positions)
    ...
    local projectile = SpawnPrefab("alterguardian_phase3trapprojectile") --天降晶石
...
local TRAP_PLAYERCOUNT_DSQ = PHASE3_TARGET_DIST^2
local function do_traps(inst, basetrapcount, minrange, maxrange)
    ...
    local num_players = 0
    local v = GetPlayer()
    --for _, v in ipairs(GetPlayer()) do
...
local PHASE3_MAXHEALTH = (TUNING.ALTERGUARDIAN_PHASE3_MAXHEALTH or 22500)
local PHASE3_STARTHEALTH = (TUNING.ALTERGUARDIAN_PHASE3_STARTHEALTH or 14000)
local HEALTH_GAIN_RATE = (PHASE3_MAXHEALTH - PHASE3_STARTHEALTH) / ((TUNING.TOTAL_DAY_TIME or 480) * 2)
...
local function fn()
  ...
  if SaveGameIndex:IsModeShipwrecked() then
    ...
    MakeCharacterPhysics(inst, 500, 0)
  end
    ...
    --inst.SetNoMusic = SetNoMusic --改，音效
    ...
    inst.SoundEmitter:PlaySound("moonstorm/creatures/boss/alterguardian3/idle_LP","idle_LP")
