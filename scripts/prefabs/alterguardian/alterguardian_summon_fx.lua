local assets =
{
    Asset("ANIM", "anim/alterguardian/alterguardian_summon_fx.zip"),
}
...
local function periodic_light_update(inst)
    local frame_num = GetCurrentAnimationFrame(inst) --inst.AnimState:GetCurrentAnimationFrame()
    --GetPlayer().components.talker:Say("动画时长："..GetCurrentAnimationFrame(inst))
