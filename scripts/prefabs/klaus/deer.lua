local assets =
{
    Asset("ANIM", "anim/klaus/deer_build.zip"),
    Asset("ANIM", "anim/klaus/deer_basic.zip"),
    Asset("ANIM", "anim/klaus/deer_action.zip"),
}

local unshackle_assets =
{
    Asset("ANIM", "anim/klaus/deer_build.zip"),
    Asset("ANIM", "anim/klaus/deer_unshackle.zip"),
}
...
local function onqueuegrowantler(inst)
    if inst.hasantler == nil and not inst.components.timer:TimerExists("growantler") then
        inst.components.timer:StartTimer("growantler", (4 + math.random() * 2) * TUNING.TOTAL_DAY_TIME)
    end
end
...
local function FindCastTargets(inst, target)
    ...
    for i, v in ipairs(TheSim:FindEntities(x, y, z, TUNING.DEER_GEMMED_CAST_RANGE, nil, SPELLTARGET_CANT_TAGS)) do
        if v.components.health and not v.components.health:IsDead() then
            ...
            elseif v.components.combat and v.components.combat.target ~= nil and v.components.combat.target:HasTag("deergemresistance") then
...
local function SetupSounds(inst)
    ...
    elseif TUNING.WINTERS_FEAST then
