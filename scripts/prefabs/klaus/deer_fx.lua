...
local function OnUpdateFade(inst)
    local k
    if inst._fade <= inst._fadeframes then
        inst._fade = math.min(inst._fade + inst._fadeinspeed, inst._fadeframes)
        k = inst._fade / inst._fadeframes
    else
        inst._fade = math.min(inst._fade + inst._fadeoutspeed, inst._fadeframes * 2 + 1)
        k = (inst._fadeframes * 2 + 1 - inst._fade) / inst._fadeframes
    end
    ...
    --if TheWorld.ismastersim then
        inst.Light:Enable(inst._fade > 0 and inst._fade <= inst._fadeframes * 2)
    --end

    if inst._fade == inst._fadeframes or inst._fade > inst._fadeframes * 2 then
...
local function OnUpdateIceCircle(inst, x, z) --冰冻
    ...
    for i, v in ipairs(TheSim:FindEntities(x, 0, z, inst._rad, nil, NOTAGS, FREEZETARGET_ONEOF_TAGS)) do
        if v:IsValid() and not (v.components.health ~= nil and v.components.health:IsDead()) then            
            ...
            if v.components.locomotor ~= nil then
                if v:HasTag("deergemresistance") then
                    ...
                    --v.components.locomotor:PushTempGroundSpeedMultiplier(TUNING.DEER_ICE_SPEED_PENALTY) --改，暂时禁用
                end
            end
            ...
            if v.components.grogginess_dst ~= nil and not v.components.grogginess_dst:IsKnockedOut() then
                local curgrog = v.components.grogginess_dst.grog_amount
                if curgrog < TUNING.DEER_ICE_FATIGUE then
                    v.components.grogginess_dst:AddGrogginess(TUNING.DEER_ICE_FATIGUE)
                end
            end
        end
    end
...
local function OnUpdateIceCircleClient(inst, x, z)
    ...
    if rad > 0 then
        ...
        if player ~= nil and
            ...
            --player.components.locomotor:PushTempGroundSpeedMultiplier(TUNING.DEER_ICE_SPEED_PENALTY) --改，暂时禁用
        end
    end
end
...
local function deer_ice_circle_common_postinit(inst)
    ...
    inst._rad = 0 --net_float(inst.GUID, "deer_ice_circle._rad")
--[[
...
local function OnUpdateFireCircle(inst, x, z) --火焰
...
local function MakeFX(name, data)
    local assets =
    {
        Asset("ANIM", "anim/klaus/"..name..".zip"),
    }
    ...
    local function fn()
        ...
        if data.light then
            ...
            inst._fade = 0 --net_smallbyte(inst.GUID, "deer_fx._fade", "fadedirty")
