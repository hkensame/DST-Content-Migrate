local assets =
{
    Asset("ANIM", "anim/klaus/klaus_basic.zip"),
    Asset("ANIM", "anim/klaus/klaus_actions.zip"),
    Asset("ANIM", "anim/klaus/klaus_build.zip"),
}
...
--]]
    local ents = TheSim:FindEntities(pos.x, pos.y, pos.z, TUNING.KLAUS_DEAGGRO_DIST)
...
local function AnnounceWarning(inst, player, strid)
    if player:IsValid() and player.entity:IsVisible() and
        ...
        --player.components.talker:Say(GetString(player, strid)) --改，暂时禁用
    end
end
...
local function MakeAKrampus(inst)
	...
	if spawn_pt then
		...
		if kramp then
			...
			kramp:FacePoint(pt)
			kramp.components.combat:SetTarget(inst.components.combat.target)
		end
	end
end
...
local function SummonHelpers(inst, warning)
    ...
    if #targets > 0 then
        ...
        for i, v in ipairs(targets) do
            ...
            GetWorld():PushEvent("ms_forcenaughtiness", { player = v.inst, numspawns = numspawns })
            stock = stock - numspawns
        end
        if warning then
            --Top_Notify:Tip(ANNOUNCE_KLAUS_CALLFORHELP)
...
local function OnMusicDirty(inst)
    ...
    if not TheNet:IsDedicated() then
        ...
        local level = inst._pausemusic:value() and 2 or (TUNING.WINTERS_FEAST:value() and 3 or 1)
...
local function Unchain(inst, warning) --死亡复活后，二阶段
    if not inst._unchained then
        ...
        --OnMusicDirty(inst)
        if warning then
            --Top_Notify:Tip(ANNOUNCE_KLAUS_UNCHAINED)
            --PushWarning(inst, "ANNOUNCE_KLAUS_UNCHAINED")
        end
    end
end

local function Enrage(inst, warning) --杀死小弟后，暴怒
    if not inst.enraged then
        ...
        if warning then
            --Top_Notify:Tip(ANNOUNCE_KLAUS_ENRAGE)
...
local function DoRemove(inst)
    GetPlayer():PushEvent("klaus_remove") --改，推送一个事件
    --Top_Notify:Tip("克劳斯已消失！")
...
local function fn()
    ...
    inst._unchained = false --net_bool(inst.GUID, "klaus._unchained", "musicdirty")
...
--]]
    ...
    local oldatked = inst.components.combat.GetAttacked
    inst.components.combat.GetAttacked = function(self,attacker,...)
    if attacker == self.inst then return end
    return oldatked(self,attacker,...)
   end
