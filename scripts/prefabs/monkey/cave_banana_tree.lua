...
-- 适配：注释 MakeNoGrowInWinter/MakeHauntableWorkAndIgnite, 移除 RemoveLunarHailBuildup
-- 返回 3 个 prefab: cave_banana_tree, cave_banana_burnt, cave_banana_stump
...
local function tree_fn()
   ...
   -- if AddToRegrowthManager then AddToRegrowthManager(inst) end -- DS 无 RegrowthManager
...
------------------------------------------------------------------------
-- 树桩 (cave_banana_stump)
------------------------------------------------------------------------

local function stump_startburn(inst)
    --blank fn to override default one
end
...
local function stump_fn()
    ...
    inst.MiniMapEntity:SetIcon("cave_banana_tree_stump.tex")
...
local function burnt_fn()
    ...
    --MakeHauntableWorkAndIgnite(inst) -- DS 无幽灵互动
