-- DS compatibility: provide polyfills for DST globals used by this module
if rawget(GLOBAL, "distsq") == nil then
    GLOBAL.distsq = function(x1, z1, x2, z2)
        local dx = x1 - x2
        local dz = z1 - z2
        return dx * dx + dz * dz
    end
end
if rawget(GLOBAL, "GetTime") == nil then
    GLOBAL.GetTime = function()
        if TheSim and TheSim.GetSimTime then
            return TheSim:GetSimTime()
        end
        return 0
    end
end
if rawget(GLOBAL, "IsEntityDeadOrGhost") == nil then
    GLOBAL.IsEntityDeadOrGhost = function(ent)
        if ent == nil then return true end
        if ent:HasTag("playerghost") then return true end
        if ent.components.health and ent.components.health:IsDead() then return true end
        return false
    end
end
if rawget(GLOBAL, "FindPlayersInRangeSortedByDistance") == nil then
    GLOBAL.FindPlayersInRangeSortedByDistance = function(x, y, z, range, includeghost)
        local players = {}
        if not AllPlayers then return players end
        for i, v in ipairs(AllPlayers) do
            if v.entity:IsVisible() and (includeghost or not v:HasTag("playerghost")) then
...
local function GetHomePosition(inst)
	return not (inst.components.follower and inst.components.follower.leader)
...
local function HasSameLeaderOrMem(a, b)
	a = (a.components.follower and a.components.follower.leader) or
		(a.components.followermemory and a:HasTag("chess") and a.components.followermemory:GetTrackingPlayer()) or
		a

	b = (b.components.follower and b.components.follower.leader) or
