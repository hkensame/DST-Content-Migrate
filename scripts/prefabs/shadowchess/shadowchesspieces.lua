...
local function createassets(name)
    return
    {
        Asset("ANIM", "anim/shadowchess/"..name..".zip"),
        Asset("ANIM", "anim/shadowchess/"..name.."_upg_build.zip"),
    }
end

local bishopfxassets =
{
    Asset("ANIM", "anim/shadowchess/shadow_bishop_fx.zip"),
}
...
local function retargetfn(inst)
    ...
    local players = TheSim:FindEntities(x, y, z, TUNING.SHADOWCREATURE_TARGET_DIST) --改FindPlayersInRange(x, y, z, TUNING.SHADOWCREATURE_TARGET_DIST, true)
...
local function commonlevelup(inst, overridelevel)
    ...
    if level ~= inst.level then
        ...
        --inst.Physics:SetCapsule(PHYS_RADIUS[inst.prefab] * scale, 1) --改，这里会造成原地移动
...
local function commonfn(name, sixfaced)
    ...
    --inst.components.health.redirect = nodmglevelingup --改，暂时禁用。不禁用三基佬会无敌的
