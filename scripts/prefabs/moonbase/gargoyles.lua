...
local function makegargoyle(data)
    local assets =
    {
        Asset("ANIM", "anim/shadowchess/sculpture_"..data.name..".zip"),
        Asset("ANIM", "anim/shadowchess/sculpture_"..data.name.."_moonrock_build.zip"),
    }
    ...
    local function fn()
        ...
        inst:SetPrefabNameOverride("gargoyle_"..data.name) --重设预设物名字
...
local function makefx(data)
    local assets =
    {
        Asset("ANIM", "anim/shadowchess/sculpture_fx.zip"),
        Asset("ANIM", "anim/moonisland/petrified_tree_fx.zip"),
    }
    ...
    local function PlayFX(proxy)
        ...
        --inst.Transform:SetFromProxy(proxy.GUID) --改，这是啥玩意？
