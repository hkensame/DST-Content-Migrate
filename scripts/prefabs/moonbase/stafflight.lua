...
local function onpulsetimedirty(inst)
    inst._pulseoffs = inst._pulsetime - inst:GetTimeAlive()
end
...
local function makestafflight(name, is_hot, anim, colour, idles, is_fx)
    ...
    local function fn()
        ...
        inst._ismastersim = true--TheWorld.ismastersim
        inst._pulseoffs = 0
        inst._pulsetime = 0--net_float(inst.GUID, "_pulsetime", "pulsetimedirty")
