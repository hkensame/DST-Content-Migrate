...
local function ToggleMoonCharge(inst)
    if inst.components.workable.workleft <= 0 then
        ...
        if GetClock() and GetClock():IsNight() and GetClock():GetMoonPhase() == "full" and
...
local function OnStaffGiven(inst, giver, item)
    ...
    if type(item) == "table" then
        ...
        skin_build = nil --item:GetSkinBuild()
    else
        staffname = item
    end
    ...
    if skin_build ~= nil then
        ...
        inst.AnimState:OverrideSymbol("swap_staffs", skin_build, GetStaffSymbol(staffname))
    ...
    if staffname == MORPHED_STAFF then
        ShowColdStar(inst)
        inst.AnimState:OverrideSymbol("swap_staffs", MORPHED_STAFF, GetStaffSymbol(MORPHED_STAFF))
    else
        HideColdStar(inst)
    end
...
local function fn()
    ...
    inst.MiniMapEntity:SetIcon("moonbase.tex")
