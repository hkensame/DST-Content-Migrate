...
local assets =
{
    Asset("ANIM", "anim/cave/mushroom_spore.zip"),
    Asset("ANIM", "anim/cave/mushroom_spore_red.zip"),
    Asset("ANIM", "anim/cave/mushroom_spore_blue.zip"),
}
...
-- Simulate DST's SetLocalMultiplier in DS
local function SetLocalMultiplier(inst, multiplier)
    if not (inst and inst.components and inst.components.perishable) then
        return
    end

    if not inst._base_perish_time then
        inst._base_perish_time = inst.components.perishable.perishtime
    end

    local perish = inst.components.perishable

    local old_percent = perish:GetPercent()

    local new_perishtime = inst._base_perish_time / multiplier

    perish:SetPerishTime(new_perishtime)
    perish:SetPercent(old_percent)
    perish:StartPerishing()
end

local function onpickup(inst)
    --These last longer when held
    SetLocalMultiplier(inst, TUNING.SEG_TIME * 3/ TUNING.PERISH_SLOW )
...
local function makespore(data)
	...
	local function fn()
        ...
        --inst.entity:AddNetwork() -- DS 无多人网络
        ...
        --inst.entity:SetPristine() -- DS 无多人网络

        -- DS 单机始终为 master sim，无需守卫

        --inst.scrapbook_anim = "flight_cycle" -- DS 无 scrapbook
        ...
        --MakeHauntablePerish(inst, .5) -- DS 无 hauntable 组件
