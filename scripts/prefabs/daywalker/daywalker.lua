...
local BONUS_PILLAR_LOOT = { "support_pillar_dreadstone_scaffold" }
...
local function EnableChains(inst, enable)
	if enable ~= inst._enablechains then
		inst._enablechains = enable
		if enable then
...
local ATTACH_POS = { "left", "right", "top" }
...
local function SetHeadTracking(inst, track)
	track = track ~= false
	if inst._headtracking ~= track then
		inst._headtracking = track
		if track then
			...
			if inst.head.stalking ~= nil then
				if not inst.head.isupdating then
					...
					inst.head.headupdatetask = inst:DoPeriodicTask(0, UpdateHead)
				end
		...
		elseif inst.head ~= nil then
			if inst.head.isupdating then
				...
				if inst.head.headupdatetask ~= nil then
					inst.head.headupdatetask:Cancel()
					inst.head.headupdatetask = nil
				end
...
local function SetStalking(inst, stalking)
	...
	if stalking ~= inst._stalking then
		...
		SetHeadTracking(inst, stalking ~= nil)
		if stalking then
...
local function OnEntityWake(inst)
	if inst._chainsleeptask ~= nil then
		inst._chainsleeptask:Cancel()
		inst._chainsleeptask = nil
	elseif inst._enablechains then
		SpawnChains(inst)
	end
	if inst._despawntask ~= nil then
...
local function fn()
	...
	inst:ListenForEvent("healthdelta", CheckPhase)
