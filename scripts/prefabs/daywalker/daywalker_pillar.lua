...
local function SpawnChains(inst)
	if inst.chains == nil then
		...
		inst.chainupdatetask = inst:DoPeriodicTask(0, OnPostUpdate)
	end
end
...
local function RemoveChains(inst, broken)
	...
	if inst.chains ~= nil then
		...
		if inst.chainupdatetask ~= nil then
			inst.chainupdatetask:Cancel()
			inst.chainupdatetask = nil
		end
	end
end
...
local function OnChainsChanged(inst)
	if inst.enablechains then
...
local function EnableChains(inst, enable)
	if enable ~= inst.enablechains then
		inst.enablechains = enable
		OnChainsChanged(inst)
	end
end

local function SetPrisoner(inst, prisoner)
	local old = inst.prisoner
...
local function OnSpawnDebris(inst)
...
local function OnWorked(inst, worker, workleft, numworks)
	...
	if changed then
		...
		inst.debrislevel = (inst.level == "med" and 2 or 1)
	else
		inst.debrislevel = 0
	end
	if not inst:IsAsleep() then
