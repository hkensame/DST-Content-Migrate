...
local function OnChange(inst)
    inst:ListenForEvent("seasonChange", function(it, data)
      if data.season ~= SEASONS.SUMMER and not inst.isdry then --不是夏季，应该干涸
		inst.components.fishable:Freeze() --不可以钓鱼
        inst.isdry = true
        ...
        inst.Physics:CollidesWith(SJ and COLLISION.WORLD or 192)
        inst.Physics:CollidesWith(COLLISION.ITEMS)

      elseif data.season == SEASONS.SUMMER and inst.isdry then --是夏季，潮湿
		inst.components.fishable:Unfreeze() --可以钓鱼
...
local function DoSpawn(inst)
    if CanSpawn(inst) then
        SpawnOasisBugs(inst) --生成萤火虫
        SpawnSucculents(inst) --多肉植物
    end
    inst.startseason = GetSeasonManager():GetSeason()
end
...
local function fn()
    ...
    inst.AnimState:PlayAnimation("idle", true) --有水
    --inst.AnimState:PlayAnimation("dry_pre") --水变干
    --inst.AnimState:PlayAnimation("wet") --干但有水渍
    --inst.AnimState:PlayAnimation("dry_idle") --干
    --inst.AnimState:PlayAnimation("dry_pst") --干变水
    inst.AnimState:SetOrientation(ANIM_ORIENTATION.OnGround)
    inst.AnimState:SetLayer(LAYER_BACKGROUND)
    inst.AnimState:SetSortOrder(2)

    inst.MiniMapEntity:SetIcon("oasislake.tex")
    ...
    inst:DoPeriodicTask(.1, function() DoSpawn(inst) end)
