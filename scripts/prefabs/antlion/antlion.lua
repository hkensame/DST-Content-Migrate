local assets =
{
    Asset("ANIM", "anim/antlion/antlion_build.zip"),
    Asset("ANIM", "anim/antlion/antlion_basic.zip"),
    Asset("ANIM", "anim/antlion/antlion_action.zip"),
    Asset("ANIM", "anim/antlion/sand_splash_fx.zip"),
}
...
local function OnGivenItem(inst, giver, item)
    ...
    --nst.components.sinkholespawner:StopSinkholes()
    ...
    if giver ~= nil and giver.components.talker ~= nil and GetTime() - (inst.timesincelasttalker or -TUNING.ANTLION_TRIBUTER_TALKER_TIME) > TUNING.ANTLION_TRIBUTER_TALKER_TIME then
        ...
        giver.components.talker:Say(ANNOUNCE_ANTLION_TRIBUTE)
    end
end
...
local function OnInit(inst)
    ...
    inst:ListenForEvent("ms_sandstormchanged", inst.onsandstormchanged, GetWorld())
    --if not (GetWorld().components.sandstorms ~= nil and GetWorld().components.sandstorms:IsSandstormActive()) then
        --Despawn(inst) --改，蚁狮消失
    --end
    --自己加一个蚁狮消失
...
local function RetargetFn(inst)
    ...
    local newplayer = GetClosestInstWithTag("character", inst, TUNING.ANTLION_CAST_RANGE)
...
local function StartCombat(inst, target, trigger)
    ...
    if inst.persists and inst.components.combat == nil then
        if inst.inittask ~= nil then
            ...
            inst:RemoveEventCallback("ms_sandstormchanged", inst.onsandstormchanged, GetWorld())
        ...
        if target ~= nil then
            ...
            if trigger == "freeze" then
                inst.components.freezable:AddColdness(999, 3)
            else
...
local function fn()
    ...
    inst.MiniMapEntity:SetIcon("antlion.tex")
