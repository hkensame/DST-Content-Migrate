...
local assets =
{
    Asset("ANIM", "anim/malbatross/malbatross_basic.zip"),
    Asset("ANIM", "anim/malbatross/malbatross_actions.zip"),    
    Asset("ANIM", "anim/malbatross/malbatross_build.zip"),
}

local rippleassets =
{
    Asset("ANIM", "anim/malbatross/malbatross_ripple.zip"),
}
...
local function PushMusic(inst)
    if GetPlayer() == nil or inst:HasTag("flight") then
        inst._playingmusic = false
    elseif GetPlayer():IsNear(inst, inst._playingmusic and 50 or 20) then
        inst._playingmusic = true
        GetPlayer():PushEvent("triggeredevent", { name = "malbatross" })
    elseif inst._playingmusic and not GetPlayer():IsNear(inst, 52) then
        inst._playingmusic = false
    end
end
...
local function Relocate(inst)
    ...
    --GetWorld().components.malbatrossspawner:Relocate(inst)
end
...
local function OnRemove(inst)
    GetWorld():PushEvent("malbatrossremoved", inst)
end
...
--]]
local function spawnwaves(inst,angle,num)
    for i = 1, 3, 1 do
    local angle_deviation = (angle/2)/(3-1)
    local fx = SpawnPrefab("whale_bubbles_fx")
      fx.Transform:SetPosition(inst.Transform:GetWorldPosition())
      fx.Transform:SetRotation(inst.Transform:GetRotation()-angle/2 + i*angle)
    end
end
...
SetSharedLootTable( 'malbatross',
{
    ...
    {'malbatross_sail_blueprint',      1.00},
...
local function fn()
    ...
    --MakeTinyFlyingCharacterPhysics(inst,1000, 1.5) --联机的鬼物理
    ...
    --inst._isengaged = net_bool(inst.GUID, "dragonfly._isengaged", "isengageddirty") --联机的东西
    inst._playingmusic = false
    inst._musictask = nil
--[[
...
--    inst:ListenForEvent("onhitother", OnHitOther)
    ...
    inst:ListenForEvent("ms_playerjoined", function(src, player) OnPlayerJoined(inst, player) end, GetWorld())
    inst:ListenForEvent("ms_playerleft", function(src, player) OnPlayerLeft(inst, player) end, GetWorld())
--[[联机的
...
local function ripplefn()
    ...
    --inst.AnimState:SetLayer(LAYER_BELOW_GROUND) --改，暂时禁用
