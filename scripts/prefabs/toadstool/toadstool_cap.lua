...
local function OnFXUpdate(inst, parent)
    local k = 1 - 1 --inst.AnimState:GetCurrentAnimationTime() / 1
    k = 1 - k * k
...
local function setnormal(inst, instant)
    ...
    if inst._dark then
        ...
        if inst._state > 0 then
            inst.MiniMapEntity:SetIcon("toadstool_cap.tex")
        end
    end
end

local function setdark(inst, duration, instant)
    if not inst._dark then
        inst._dark = true
        inst.AnimState:SetBuild("toadstool_dark_build")
        if inst._state > 0 then
            inst.MiniMapEntity:SetIcon("toadstool_cap_dark.tex")
        end
    end
    if not instant then
...
local function onspawntoad(inst)
    ...
    local toadstool = SpawnPrefab(inst._dark and "toadstool_dark" or "toadstool")
...
local function ongrown(inst)
    ...
    inst.MiniMapEntity:SetIcon(inst._dark and "toadstool_cap_dark.tex" or "toadstool_cap.tex")
...
setstate = function(inst, state)
    ...
    if state ~= inst._state then
        ...
        if state == 0 then
            ...
            setnormal(inst, true)
            inst.MiniMapEntity:SetIcon("toadstool_hole.tex")
        ...
        inst._state = state
        MakeObstaclePhysics(inst, 0) --改，砍蘑菇后的物理，我加�?
        inst:PushEvent("toadstoolstatechanged", state)
    end
end

local function getstatus(inst)
    return (inst._state == 0 and "EMPTY")
        or (inst._state == 1 and "INGROUND")
        or nil
end

local function displaynamefn(inst)
    return (inst._state == 0 and STRINGS.NAMES.TOADSTOOL_HOLE)
        or (inst._dark and STRINGS.NAMES.TOADSTOOL_CAP_DARK)
        or nil
end

local function onsave(inst, data)
    data.state = inst._state > 0 and inst._state or nil
end
...
local function onloadpostpass(inst)
    ...
    if darktime > 0 and inst._state > 0 then
...
local function hastoadstool(inst)
    return inst._state > 0 or inst.components.entitytracker:GetEntity("toadstool") ~= nil
end
...
local function fn()
    ...
    inst._state = 0 --net_tinybyte(inst.GUID, "toadstool_cap._state")
    inst._dark = true --net_bool(inst.GUID, "toadstool_cap._dark")
