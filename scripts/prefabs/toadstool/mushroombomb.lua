...
local function Explode(inst)
    ...
    local x, y, z = inst.Transform:GetWorldPosition() --最开始的小蘑菇爆�?
    local ents = TheSim:FindEntities(x, y, z, TUNING.TOADSTOOL_MUSHROOMBOMB_RADIUS, nil, EXPLODETARGET_CANT_TAGS)
    if #ents > 0 then
...
local function Grow(inst, level)
    if level > 2 then
        ...
        inst._growtask = inst:DoTaskInTime(1, Grow, level + 1)
...
local function MakeBomb(name)
    local assets =
    {
        Asset("ANIM", "anim/toadstool/mushroombomb.zip"),
        Asset("ANIM", "anim/toadstool/mushroombomb_base.zip"),
    }
    if name ~= "mushroombomb" then
        table.insert(assets, Asset("ANIM", "anim/toadstool/"..name.."_build.zip"))
    end
    ...
    local function fn()
        ...
        inst._fade = 0 --net_smallbyte(inst.GUID, "mushroombomb._fade", "fadedirty")
        ...
        inst._soundtask = nil
        inst._growtask = inst:DoTaskInTime(1, Grow, 1)
...
local function MakeProjectile(name, bombname)
    ...
    if bombname ~= "mushroombomb" then
        table.insert(assets, Asset("ANIM", "anim/toadstool/"..bombname.."_build.zip"))
    end
