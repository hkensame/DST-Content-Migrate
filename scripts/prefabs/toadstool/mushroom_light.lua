...
local function mushroom_light2itemtestfn(container, item, slot)
    return (item:HasTag("lightbattery") or item:HasTag("spore") or item:HasTag("lightcontainer")) --and not container.inst:HasTag("burnt")
end

local function mushroom_lightitemtestfn(container, item, slot)
...
local function MakeMushroomLight(name, onlywhite, physics_rad)
    local assets =
    {
        Asset("ANIM", "anim/toadstool/"..name..".zip"),
        Asset("ANIM", "anim/toadstool/ui_lamp_1x4.zip"),
    }
    ...
    local function fn()
		...
		if name == "mushroom_light2" then 
			inst.components.container.itemtestfn = mushroom_light2itemtestfn
		else
			inst.components.container.itemtestfn = mushroom_lightitemtestfn
		end
...
-- STRINGS 已在 dst_strings.lua 中定义（中文）
