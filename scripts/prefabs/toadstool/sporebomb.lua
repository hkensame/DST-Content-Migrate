local assets =
{
    Asset("ANIM", "anim/toadstool/sporebomb.zip"),
}
...
local function OnUpdateFade(inst)
    ...
    inst._light.Light:Enable(inst._fade > 0 and inst._fade <= FADE_FRAMES * 2)
...
local function OnAttached(inst, target, followsymbol, followoffset)
    --inst.entity:SetParent(target.entity) --改，蘑菇让禁用
...
local function fn()
    ...
    inst._fade = 0 --net_smallbyte(inst.GUID, "sporebomb._fade", "fadedirty")
