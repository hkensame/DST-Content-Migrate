local assets =
{
    Asset("ANIM", "anim/toadstool/sporecloud.zip"),
    Asset("ANIM", "anim/toadstool/sporecloud_base.zip"),
}
...
local function fn()
    ...
    inst._state = 0 --net_tinybyte(inst.GUID, "sporecloud._state", "statedirty")
    inst._fade = 0 --net_smallbyte(inst.GUID, "sporecloud._fade", "fadedirty")
    ...
    --inst.components.aura.auraexcludetags = function(inst, target) --AURA_EXCLUDE_TAGS
    inst.components.aura.auratestfn = function(inst, target)
      for i = 1, (#AURA_EXCLUDE_TAGS) do
        if target:HasTag(AURA_EXCLUDE_TAGS[i]) then
          return false
        end
      end
      return true
