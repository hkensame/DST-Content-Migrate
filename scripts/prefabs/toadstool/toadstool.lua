...
local function OnUpdateFade(inst)
    local k
    if inst._fade <= FADE_FRAMES then
        inst._fade = math.min(inst._fade + 1, FADE_FRAMES)
        k = inst._fade / FADE_FRAMES
    else
        inst._fade = math.min(inst._fade + 1, FADE_FRAMES * 2 + 1)
        k = (FADE_FRAMES * 2 + 1 - inst._fade) / FADE_FRAMES
    end
    ...
    --if TheWorld.ismastersim then
        inst.Light:Enable(inst._fade > 0 and inst._fade <= FADE_FRAMES * 2)
    --end

    if inst._fade == FADE_FRAMES or inst._fade > FADE_FRAMES * 2 then
...
local function FindSporeBombTargets(inst, preferredtargets)
    ...
    local ents = TheSim:FindEntities(x, y, z, TUNING.TOADSTOOL_SPOREBOMB_ATTACK_RANGE, nil, SPOREBOMBTARGET_CANT_TAGS)
...
local function FindMushroomBombTargets(inst)
    ...
    for i = 1, 2 do
        ...
        local targets = TheSim:FindEntities(pt.x, 0, pt.z, closerange, nil, BOMBTARGET_CANT_TAGS)
...
--为什么老是组建报空值啊
    local map = GetWorld().Map --TheWorld.Map
    local pt = inst:GetPosition()
    local theta = table.remove(angles, math.random(#angles))
    ...
    local min_spacing = 2 --DEPLOYSPACING_RADIUS[DEPLOYSPACING.DEFAULT]
    ...
    for i = 1, 12 do
        ...
        if offset ~= nil then
            ...
            if #TheSim:FindEntities(pt.x, 0, pt.z, min_spacing, nil, MUSHROOMSPROUT_BLOCKER_TAGS) <= 0 then
                ...
                local totoss = TheSim:FindEntities(pt.x, 0, pt.z, 1, nil, MUSHROOMSPROUT_TOSS_CANT_TAGS)

                --toss flowers out of the way
                for i, v in ipairs(TheSim:FindEntities(pt.x, 0, pt.z, 1, nil, MUSHROOMSPROUT_TOSSFLOWERS_CANT_TAGS)) do
                  if v.components.pickable then --改，加的判定
...
--]]
    local ents = TheSim:FindEntities(pos.x, pos.y, pos.z, TUNING.TOADSTOOL_DEAGGRO_DIST)
...
local function RetargetFn(inst)
	return FindEntity(inst, TUNING.TOADSTOOL_ATTACK_RANGE, 
		function(guy) return inst.components.combat:CanTarget(guy) end, nil, 
...
local function AnnounceWarning(inst, player)
    if player:IsValid() and player.entity:IsVisible() and
        ...
        player.components.talker:Say(ANNOUNCE_TOADESCAPING)
    end
end
...
local function AnnounceEscaped(player)
    if player:IsValid() and player.entity:IsVisible() and
        ...
        not player:HasTag("playerghost") and
        player.components.talker ~= nil then
        player.components.talker:Say(ANNOUNCE_TOADESCAPED)
    end
end
...
local function SetPhaseLevel(inst, phase)
    ...
    if phase > 2 then
        inst.components.timer:ResumeTimer("pound_cd") --改，暂时禁用
    else
...
local function DropShroomSkin(inst)
    local player--[[, rangesq]] = GetPlayer() --inst:GetNearestPlayer()
...
local function common_fn(build)
    ...
    inst._fade = 0 --net_smallbyte(inst.GUID, "toadstool._fade", "fadedirty")
--[[
