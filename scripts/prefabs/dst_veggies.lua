-- DST 移植蔬菜（洋葱、辣椒）及其种子、熟食，追加到原版 VEGGIES 表
require "tuning"
...
for k, v in pairs(DST_VEGGIES) do
  VEGGIES[k] = v
end
...
local function ondeploy(inst, pt, deployer)
    ...
    local plant_prefab = "plant_normal"
    local prefab = nil
    ...
    if GetPlayer():HasTag("plantkin") then
      plant_prefab = "plant_normal_ground"
    end

    local plant1 = SpawnPrefab(plant_prefab)
...
local function MakeVeggie(name, has_seeds, iswater)
  ...
  inst.components.inventoryitem.atlasname = "images/dst_boss.xml"
	...
	local function fn(Sim)
		...
		inst.components.edible.healthvalue = DST_VEGGIES[name].health
		inst.components.edible.hungervalue = DST_VEGGIES[name].hunger
		inst.components.edible.sanityvalue = DST_VEGGIES[name].sanity or 0
		inst.components.perishable:SetPerishTime(DST_VEGGIES[name].perishtime)
  ...
  inst.components.inventoryitem.atlasname = "images/dst_boss.xml"
	...
	local function fn_cooked(Sim)
		...
		inst.components.perishable:SetPerishTime(DST_VEGGIES[name].cooked_perishtime)
		inst.components.edible.healthvalue = DST_VEGGIES[name].cooked_health
		inst.components.edible.hungervalue = DST_VEGGIES[name].cooked_hunger
		inst.components.edible.sanityvalue = DST_VEGGIES[name].cooked_sanity or 0
  ...
  inst.components.inventoryitem.atlasname = "images/dst_boss.xml"
		...
		if rawget(_G, 'MakeInventoryFloatable') then
		    MakeInventoryFloatable(inst, "cooked", "cooked")
		end
		---------------------
...
local prefs = {}
for veggiename,veggiedata in pairs(DST_VEGGIES) do
