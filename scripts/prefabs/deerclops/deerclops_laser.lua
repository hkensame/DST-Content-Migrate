local assets =
{
    Asset("ANIM", "anim/deerclops/deerclops_laser_hit_sparks_fx.zip"),
}
...
local function DoDamage(inst, targets, skiptoss)
    ...
    for i, v in ipairs(TheSim:FindEntities(x, 0, z, RADIUS + 3, nil, DAMAGE_CANT_TAGS)) do
    ...
    for i, v in ipairs(TheSim:FindEntities(x, 0, z, RADIUS + 3, nil, LAUNCH_CANT_TAGS)) do
...
local function Scorch_OnFadeDirty(inst)
    ...
    if inst._fade > SCORCH_FADE_FRAMES + SCORCH_DELAY_FRAMES then
        local k = (inst._fade - SCORCH_FADE_FRAMES - SCORCH_DELAY_FRAMES) / SCORCH_RED_FRAMES
        inst.AnimState:OverrideMultColour(1, 1, 1, 1)
        inst.AnimState:SetHighlightColour(k, 0, 0, 0)
    elseif inst._fade >= SCORCH_FADE_FRAMES then
        inst.AnimState:OverrideMultColour(1, 1, 1, 1)
        inst.AnimState:SetHighlightColour()
    else
        local k = inst._fade / SCORCH_FADE_FRAMES
        k = k * k
...
local function Scorch_OnUpdateFade(inst)
    ...
    --elseif TheWorld.ismastersim then --改，先禁用
        --inst:Remove()
    elseif inst._fade > 0 then
        inst._fade = 0
...
local function scorchfn()
    ...
    inst._fade = 0 --net_byte(inst.GUID, "deerclops_laserscorch._fade", "fadedirty")
    inst._fade = (SCORCH_RED_FRAMES + SCORCH_DELAY_FRAMES + SCORCH_FADE_FRAMES)

    --inst:DoPeriodicTask(0, Scorch_OnUpdateFade)
    inst:DoPeriodicTask(2, function(inst) inst:Remove() end)
    --Scorch_OnFadeDirty(inst)
--[[
