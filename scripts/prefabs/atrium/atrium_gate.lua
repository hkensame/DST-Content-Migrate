...
local assets =
{
    Asset("ANIM", "anim/atrium/atrium_gate.zip"),
    Asset("ANIM", "anim/atrium/atrium_floor.zip"),
...
local function OnCameraFocusDirty(inst)
    if inst._camerafocus > 0 then
        if inst._camerafocus <= 1 then
...
local function SetCameraFocus(inst, level)
    if level ~= inst._camerafocus then
        inst._camerafocus = (level)
...
local SUPPRESS_SHADOWS_MUST_TAGS = { "health" }
...
local function OnKeyGiven(inst, giver)
    ...
    inst.MiniMapEntity:SetIcon("atrium_gate_active.tex")

    GetWorld():PushEvent("atriumpowered", true)
    GetWorld():PushEvent("ms_locknightmarephase", "wild")
    GetWorld():PushEvent("pausequakes", { source = inst })
    GetWorld():PushEvent("pausehounded", { source = inst })

    if giver ~= nil then
        inst.SoundEmitter:PlaySound("dontstarve/common/together/atrium_gate/key_in")

--      if giver.components.talker ~= nil then
--          giver.components.talker:Say(GetString(giver.prefab, "ANNOUNCE_GATE_ON"))
--      end
    end
end
...
local function OnKeyTaken(inst)
    ...
    GetWorld():PushEvent("atriumpowered", false)
    GetWorld():PushEvent("ms_locknightmarephase", nil)
    GetWorld():PushEvent("unpausequakes", { source = inst })
    GetWorld():PushEvent("unpausehounded", { source = inst })
end

local function DoPlayerWarning(inst, player)
    if player:IsValid() then
        player.components.talker:Say(GetString(player.prefab, "ANNOUNCE_ATRIUM_DESTABILIZING"))
    end
end
...
GetPlayer().components.talker:Say("2222")
    inst.components.trader:Disable()
    inst.components.pickable.caninteractwith = false
    inst:RemoveTag("intense")
...
local function OnTrackStalker(inst, stalker)
    if stalker.components.health ~= nil and not stalker.components.health:IsDead() then
        ...
        SetCameraFocus(inst, 0)
        EnableShadowSuppression(inst, false)
        GetPlayer().components.talker:Say("4444444")
        ShowFx(inst, "idle")
        inst.AnimState:PlayAnimation("idle_fight", true)
        inst.SoundEmitter:KillSound("loop")
        inst.SoundEmitter:PlaySound("dontstarve/common/together/atrium_gate/active_LP", "loop")
...
local function CreateTerraformBlocker(parent)
    ...
    --inst:SetTerraformExtraSpacing(TERRAFORM_BLOCKER_RADIUS) --改，暂时禁用
...
--ShowFx(inst, "overload") --改
    ...
    inst._camerafocus = 0 --net_tinybyte(inst.GUID, "atrium_gate._camerafocus", "camerafocusdirty")
    inst._camerafocustask = nil
...
--]]
	...
	inst.components.trader:SetAcceptTest(ItemTradeTest) --inst.components.trader:SetAbleToAcceptTest(ItemTradeTest)
