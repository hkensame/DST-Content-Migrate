local assets =
{
    Asset("ANIM", "anim/atrium/fossil_spike.zip"),
}
...
local function DoDamage(inst)
    ...
    local totoss = TheSim:FindEntities(x, 0, z, PHYSICS_RADIUS + DAMAGE_RADIUS_PADDING, nil, TOSSITEM_CANT_TAGS)
...
--MakeObstaclePhysics(inst, 0.5, 2)
    ...
    inst.Physics:SetCollisionGroup(COLLISION.SMALLOBSTACLES or 8192)
    inst.Physics:ClearCollisionMask()
    inst.Physics:CollidesWith(COLLISION.ITEMS)
    inst.Physics:CollidesWith(COLLISION.CHARACTERS)
