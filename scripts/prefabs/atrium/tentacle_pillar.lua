...
local function SwapToHole(inst)
    ...
    local hole = SpawnPrefab("tentacle_pillar_hole") if hole == nil then return end
...
local function fn()
    ...
    inst.MiniMapEntity:SetIcon("tentacle_pillar.tex")
...
-- 布局表仅在 modworldgenmain 中预注册，加载中尚未注册则不收集清理排除列表
local ok, tentacle_layout = pcall(obj_layout.LayoutForDefinition, "TentaclePillarToAtrium")
if ok and tentacle_layout then
...
local function PlaceLayout(layout, prefabs, position)
    local rcx = math.floor(position[1] - #layout.ground/2) - 1
...
local function AtriumTentacle_OnLoad(inst, data)
    if data == nil or not data.spawn_outer then
        return
    end
...
local function atrium_fn()
    ...
    inst.OnLoad = AtriumTentacle_OnLoad
