local assets_cave =
{
    Asset("ANIM", "anim/atrium/stalker_basic.zip"),
    Asset("ANIM", "anim/atrium/stalker_action.zip"),
    Asset("ANIM", "anim/atrium/stalker_shadow_build.zip"),
    Asset("ANIM", "anim/atrium/stalker_cave_build.zip"),
}

local assets_forest =
{
    Asset("ANIM", "anim/atrium/stalker_forest.zip"),
    Asset("ANIM", "anim/atrium/stalker_shadow_build.zip"),
    Asset("ANIM", "anim/atrium/stalker_forest_build.zip"),
}
...
local assets_atrium =
{
    Asset("ANIM", "anim/atrium/stalker_basic.zip"),
    Asset("ANIM", "anim/atrium/stalker_action.zip"),
    Asset("ANIM", "anim/atrium/stalker_atrium.zip"),
    Asset("ANIM", "anim/atrium/stalker_shadow_build.zip"),
    Asset("ANIM", "anim/atrium/stalker_atrium_build.zip"),
}
...
local function EnableCameraFocus(inst, enable)
    if enable ~= inst._camerafocus then
        inst._camerafocus = enable
        --if not TheNet:IsDedicated() then --改，暂时禁用
...
--]]
    local ents = TheSim:FindEntities(x, y, z, TUNING.STALKER_DEAGGRO_DIST)
...
local RETARGET_MUST_TAGS = { "combat", "locomotor" }
...
local function AtriumBattleChatter(inst, id, forcetext) --改，暂时禁用
...
local SNARE_TAGS = { "combat", "locomotor" }
local SNARE_NO_TAGS = { "flying", "ghost", "tallbird", "fossil", "shadow", "shadowminion", "INLIMBO", "epic", "smallcreature" }
local function FindSnareTargets(inst)
    ...
    local inatrium = inst.atriumstalker and inst:IsNearAtrium()
    local ents = TheSim:FindEntities(x, y, z, TUNING.STALKER_SNARE_RANGE, nil, SNARE_TAGS, SNARE_NO_TAGS)
    for i, v in ipairs(ents) do
        if not (v.components.health ~= nil and v.components.health:IsDead()) and
            ...
            elseif v.components.combat and v.components.combat:TargetIs(inst) then
...
local function SpawnSnare(inst, x, z, r, num, target)
    ...
    for theta = math.random() * dtheta, PI * 2, dtheta do
        ...
        if IsPassableAtPoint(x1, 0, z1) then --and not IsPointNearHole(Vector3(x1, 0, z1)) then
...
local function SpawnSnares(inst, targets)
    ...
    for i, v in ipairs(targets) do
        if v:IsValid() and
            ...
            local r = v:GetPhysicsRadius(0) + (islarge and 1.5 or .4) --改，原是0.5但困不住人，所以改小了
...
local function DoSpawnChanneler(inst)
    ...
    if IsAboveGroundAtPoint(x, 0, z) then
    ...
    for ring = 1, NUM_RINGS do
        ...
        if ringcount > 0 then
            ...
            for i = 1, ringcount do
                ...
                if IsAboveGroundAtPoint(x1, 0, z1) then
...
local function GenerateSpiralSpikes(inst)
    ...
    for i = 1, num do
        ...
        if IsPassableAtPoint(x1, 0, z1) then
...
local function IsValidMindControlTarget(inst, guy, inatrium)
    ...
    return not (guy.components.health:IsDead() or guy:HasTag("playerghost"))
        and (guy:DebuffsEnabled()) --改，暂时禁用
        and guy.entity:IsVisible()
end

local function IsCrazyGuy(guy)
    local sanity = guy ~= nil and guy.components.sanity or nil
    return sanity ~= nil and sanity:GetPercent() <= (guy:HasTag("dappereffects") and TUNING.DAPPER_BEARDLING_SANITY or TUNING.BEARDLING_SANITY)
end
...
local function OnLostAtrium(inst) --改，暂时禁用
    if not inst.components.health:IsDead() then
...
local function DoPlantBloom(inst)
    ...
    local offset = FindValidPositionByFan(
        ...
        function(offset)
            local x1 = x + offset.x
            local z1 = z + offset.z
            return IsPassableAtPoint(x1, 0, z1)
                and IsDeployPointClear(Vector3(x1, 0, z1), nil, 1)
...
local function common_fn(bank, build, shadowsize, canfight, atriumstalker)
    ...
    MakeCharacterPhysics(inst, 1000, .75) --MakeGiantCharacterPhysics(inst, 1000, .75)
    ...
    if atriumstalker then
        ...
        inst._camerafocus = false --net_bool(inst.GUID, "stalker._camerafocus", "camerafocusdirty")
        inst._camerafocustask = nil
...
--]]
    ...
    if canfight then
        ...
        --inst.components.combat.GetBattleCryString = atriumstalker and AtriumBattleCry or BattleCry --改，暂时禁用
...
local function forest_fn()
    ...
    --inst:WatchWorldState("isnight", OnIsNight) --改，暂时禁用
...
local function atrium_fn()
    ...
    --改，用这个会无敌inst.components.health.redirect = nodmgshielded
