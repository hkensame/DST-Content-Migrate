local assets =
{
    Asset("ANIM", "anim/atrium/stalker_shield.zip"),
}
...
local function StartRepel(inst)
    ...
    local creatures = {}
    for i, v in ipairs(TheSim:FindEntities(x, y, z, REPEL_RADIUS, nil, SLEEPREPEL_CANT_TAGS)) do
...
local function MakeShield(name, num, prefabs)
    local function fn()
        ...
        inst:DoTaskInTime((SJ and 1 or inst.AnimState:GetCurrentAnimationLength()) + FRAMES, inst.Remove)
