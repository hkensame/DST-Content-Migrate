local assets =
{
    Asset("ANIM", "anim/atrium/fossil_stalker.zip"),
}
...
local function ItemTradeTest(inst, item, giver)
    ...
    elseif GetClosestInstWithTag("gemsocket", inst, 10) --giver.components.areaaware:CurrentlyInTag("Atrium")
...
local function OnAccept(inst, giver, item)
    if item.prefab == "shadowheart" then
        ...
        elseif not GetClosestInstWithTag("gemsocket", inst, 20) then --改，先凑合用giver.components.areaaware:CurrentlyInTag("Atrium") then
...
local function makemound(name)
    local function fn()
        ...
        inst.components.repairable.repairmaterial = "fossil"
