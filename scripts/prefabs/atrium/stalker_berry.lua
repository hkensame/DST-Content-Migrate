local assets =
{
    Asset("ANIM", "anim/atrium/forest_glowberry.zip"),
}
...
local function FadeOut(inst, instant)
    if instant then
        ...
        OnFadeDirty(inst)
    elseif inst._fade <= FADE_FRAMES then
        inst._fade = FADE_FRAMES * 2 + 1 - inst._fade
        if inst._fadetask == nil then
            inst._fadetask = inst:DoPeriodicTask(FRAMES, OnUpdateFade)
        end
    end
...
local function fn()
    ...
    inst._fade = 0--net_smallbyte(inst.GUID, "stalker_berry._fade", "fadedirty")
    inst._fadetask = inst:DoPeriodicTask(FRAMES, OnUpdateFade)
