local assets =
{
    Asset("ANIM", "anim/dragonfly/lavae_egg.zip"),
...
local function OnNear(inst)
    --print("lavaeegg - OnNear")
    inst.playernear = true
    CheckHatch(inst)
end

local function OnFar(inst)
    --print("lavaeegg - OnFar")
...
local function OnHatchState(inst, state)
    if state == "crack" then
        ...
        inst:DoTaskInTime(22 * FRAMES, function() 
		inst.SoundEmitter:PlaySound("dontstarve/creatures/together/lavae/egg_bounce") 
		end)
    ...
    elseif state == "uncomfy" then
	    ...
	    elseif inst.components.hatchable.toohot then
            inst.AnimState:PlayAnimation("idle_cold", true)
            inst.SoundEmitter:PlaySound("dontstarve/creatures/together/lavae/egg_shiver")
        end
    ...
    elseif state == "dead" then
		...
		inst.SoundEmitter:PlaySound("dontstarve/creatures/together/lavae/egg_deathpoof") 
		end)
        inst:DoTaskInTime(43 * FRAMES, function() 
		inst.SoundEmitter:PlaySound("dontstarve/creatures/together/lavae/egg_deathcrack") 
		end)
...
local function common_fn(anim)
	...
	elseif anim and anim == "happy" then 
	inst.components.inventoryitem.imagename = "lavae_egg_cracked"
	inst.components.inventoryitem.atlasname = "images/dst_boss.xml"
	end
