-- DS 适配版 mushgnome.lua
-- 从 DST 源码 scripts/prefabs/mushgnome.lua 移植
-- 改动：
--   🔴 移除 AddNetwork / SetPristine / ismastersim 守卫
--   🔴 移除 drownable 组件（DS 无海洋系统）
--   🔴 注释 MakeHauntableIgnite
--   ⚪ scrapbook_* 字段无害保留
...
local function normal_fn()
    ...
    -- ⚠️ DS 的 MakeCharacterPhysics 返回 nil，不要捕获返回值
    ...
    inst.scrapbook_anim = "idle_loop" -- ⚪ 无害保留

    -- 🔴 DS 不需要 SetPristine / ismastersim 守卫

    inst.scrapbook_damage = 0 -- ⚪ 无害保留
    ...
    -- 🔴 移除 inst:AddComponent("drownable")（DS 无海洋系统）

    ------------------------------------------
    inst:SetStateGraph("SGmushgnome")
    -- ⚪ burn_on_electrocute 在 DS 中无对应 SG 状态，无害赋值
    ...
    ------------------------------------------
    -- 🔴 注释 MakeHauntableIgnite（DS 无幽灵系统）
