-- mushtree_moon.lua - 月蘑菇树（移植自 DST）
-- Factory pattern: 生成 mushtree_moon + mushtree_moon_stump + mushtree_moon_burntfx
...
local REMOVABLE = { ["log"] = true, ["moon_cap"] = true, ["charcoal"] = true }
        ...
        if inst:IsAsleep() then
            ...
            for i, v in ipairs(TheSim:FindEntities(x, y, z, 6, DECAYREMOVE_MUST_TAGS, DECAYREMOVE_CANT_TAGS)) do
                if REMOVABLE[v.prefab] then
                    if leftone then v:Remove() else leftone = true end
                end
            end
        else
...
local function maketree(name, data, state)
    local function normal_tree(inst, instant)
        if inst._changetask ~= nil then inst._changetask:Cancel() end
        if instant then
    ...
    local function makestump(inst)
        if inst:HasTag("stump") then return end
        if inst._changetask ~= nil then inst._changetask:Cancel(); inst._changetask = nil end
        ...
        inst.MiniMapEntity:SetIcon("mushroom_tree_stump.tex")
    ...
    local function onload(inst, loaddata)
        if loaddata ~= nil then
            if loaddata.burnt then
                if loaddata.stump then stump_burnt(inst) else tree_burnt(inst) end
...
function treeset(name, data, build_file_name)
    ...
    local prefabs = {
        ...
        "ash", "charcoal", "log", "small_puff", "moon_cap",
    }
...
treeset("mushtree_moon", data.moon, "anim/cave/mutatedmushroom_tree_build.zip")
