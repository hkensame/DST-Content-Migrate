...
local function addcanopyrow(widget,row)
    ...
    for i=1,#POS_INDEX do
        ...
        widget["leavesTop"..row.."_"..i]:GetAnimState():SetBuild("dst_leaves_canopy")        
        widget["leavesTop"..row.."_"..i]:SetScaleMode(SCALEMODE_PROPORTIONAL)
        --widget["leavesTop"..row.."_"..i]:GetAnimState():SetEffectParams(2.0, 4.0, 4.0, 20.0)  
...
function Leafcanopy:OnUpdate(dt)
    local tree = FindEntity(GetPlayer(), 25, nil, {"shadecanopysmall"})
    ...
    self.under_leaves =  true--self.owner._underleafcanopy and self.owner._underleafcanopy:value()
