...
local function GoHomeAction(inst)
    ...
    if home ~= nil and home.components ~= nil and ((home.components.burnable ~= nil and home.components.burnable:IsBurning()) or
...
function SpiderBrain:OnStart()
    ...
    local post_nodes = PriorityNode({
        ...
        WhileNode(function() return (GetClock():IsDay() or self.inst._quaking) and not self.inst.summoned end, "IsDay",
    ...
    local attack_nodes = PriorityNode({
        IfNode(function() return not self.inst.bedazzled and (self.inst.components.follower == nil or self.inst.components.follower.leader == nil) end, "AttackWall",
    ...
    local follow_nodes = PriorityNode({
        ...
        IfNode(function() return self.inst.components.follower ~= nil and self.inst.components.follower.leader ~= nil end, "HasLeader",
            FaceEntity(self.inst, GetFaceTargetFn, KeepFaceTargetFn )),
    })
