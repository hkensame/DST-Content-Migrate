...
local START_FACE_DIST = (TUNING.ALTERGUARDIAN_PHASE3_ATTACK_RANGE or 14) + 3
local function GetFaceTargetFn(inst)
    local target = inst.components.combat.target or FindClosestPlayerToInst(inst, START_FACE_DIST, true)
    return target ~= nil and not target:HasTag("notarget") and target or nil
end

local KEEP_FACE_DIST = (TUNING.ALTERGUARDIAN_PHASE3_ATTACK_RANGE or 14) + 3
...
local function GetHunter(inst)
    return FindEntity(inst, AVOID_PLAYER_DIST + 5, nil, {"character", "monster"}, {"INLIMBO", "playerghost"})
end
...
function AlterGuardian_Phase3Brain:OnStart()
    local root = PriorityNode({
        ...
        WhileNode(function() return not self.inst.components.timer:TimerExists("runaway_blocker") end, "Run Away",
            RunAway(self.inst, GetHunter, AVOID_PLAYER_DIST, AVOID_PLAYER_STOP)
        ),
