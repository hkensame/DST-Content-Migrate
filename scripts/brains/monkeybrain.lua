-- 普通猴子 AI (monkeybrain)
-- 移植自 DST，适配 DS 单机模式
-- 移除：BrainCommon.PanicTrigger / ElectricFencePanicTrigger (mod的braincommon不含这些)
-- 适配：酸雨 WhileNode 移除(DS无酸雨)
...
-- DS 无 HasAnyTag 方法，用 HasTag 循环替代
local function hasAnyTag(entity, tags)
    for i = 1, #tags do
        if entity:HasTag(tags[i]) then return true end
    end
    return false
end
local function AnnoyLeader(inst)
    ...
    local lootchests = inst.canlootchests ~= false
    ...
    if ba ~= nil and ba.action.id == "PICKUP" then
        ...
        if tar ~= nil and tar:IsValid() and
                ...
                not hasAnyTag(tar, CANT_PICKUP_TAGS) and
...
function MonkeyBrain:OnStart()
    ...
    {
        -- BrainCommon.PanicTrigger / ElectricFencePanicTrigger 在 mod braincommon 中不存在，移除

        -- 猴子在地震时回家
        EventNode(self.inst, "gohome",
            DoAction(self.inst, GoHome)),
        -- 酸雨 WhileNode 移除 (DS 无酸雨)
