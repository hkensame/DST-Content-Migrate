...
local function ResetData(inst)
    if GetWorld().components.deerherding ~= nil then
       GetWorld().components.deerherding:SetHerdAlertTarget(inst, FindClosestPlayerToInst(inst, START_ALERT_DIST, true))
    end
end

local function GetAlertTargetFn(inst)
    if inst:IsValid() then
        return GetWorld().components.deerherding:GetClosestHerdAlertTarget(inst)
    end
end

local function KeepAlertTargetFn(inst, target)
    if inst:IsValid() then
        return GetWorld().components.deerherding:HerdHasAlertTarget()
    end
end
...
local function GetLocationInHerd(inst)
    local herdpt = GetWorld().components.deerherding.herdlocation
...
local function IsHerdGrazing(self)
    return GetWorld().components.deerherding:IsGrazing()
    -- todo: Ask GetWorld().components.deerherding if it is grazing
end
...
function DeerBrain:OnStart()
    local solomentality = PriorityNode(
    {
        ...
        WhileNode(function() return self.inst.components.combat:HasTarget() end, "Flee", 
            PriorityNode{
                ...
                RunAway(self.inst, function() return self.inst.components.combat.target end, TUNING.DEER_ATTACKER_REMEMBER_DIST, TUNING.DEER_ATTACKER_REMEMBER_DIST),
    ...
    PriorityNode(
    {
        ...
        WhileNode(function() return GetWorld().components.deerherding ~= nil and GetWorld().components.deerherding:IsActiveInHerd(self.inst) end, "Herd Mentality",
            herdmentality
        ),
        solomentality,
