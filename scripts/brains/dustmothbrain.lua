-- DS 适配版：移除 BrainCommon.PanicTrigger / ElectricFencePanicTrigger
-- DST 的 BrainCommon 不包含这些函数，直接移除 panic 节点
...
local HUNTERPARAMS_NOPLAYER = { "scarytoprey" }

local function FindScaryPrey(inst)
    return FindEntity(inst, SEE_THREAT_DIST, nil, HUNTERPARAMS_NOPLAYER, { "player", "NOCLICK" })
end
...
local function DustOffAction(inst)
    -- DS: ACTIONS.PET is DST-only, nil in DS
    if ACTIONS.PET == nil then
        return
    end
...
function DustMothBrain:OnStart()
    local root = PriorityNode(
    {
        ...
        RunAway(self.inst, FindScaryPrey, SEE_THREAT_DIST, STOP_RUN_DIST),
