...
local Dragonfly2Brain = Class(Brain, function(self, inst)
...
local function ShouldResetFight(self)
    if not self.inst.reset then
        if self.inst:GetDistanceSqToPoint(self.inst.components.knownlocations:GetLocation("spawnpoint")) >= TUNING.DRAGONFLY2_RESET_DIST * TUNING.DRAGONFLY2_RESET_DIST then
...
local function ShouldSpawnFn(self)
    ...
    elseif self._spawnpos == nil then
        ...
        local lavae_ponds = TheSim:FindEntities(pos.x, pos.y, pos.z, TUNING.DRAGONFLY2_RESET_DIST, { "lava" })
...
function Dragonfly2Brain:OnSpawnLavae()
    self._spawnpos = nil
    self.inst.components.rampingspawner:SpawnEntity()
end

function Dragonfly2Brain:OnStart()
    local root =
        PriorityNode(
        {
