...
-- DS RunAway 不支持 table 参数，改用 function 形式
-- 原 DST: {oneoftags = {"nightmarecreature", "shadowcreature", ...}}
local SHADOW_TAGS_LIST = {"nightmarecreature", "shadowcreature", "shadow", "shadowminion", "stalker", "stalkerminion", "nightmare", "shadow_fire"}
local function IsShadowCreature(ent)
    for _, tag in ipairs(SHADOW_TAGS_LIST) do
        if ent:HasTag(tag) then return true end
    end
    return false
end
    ...
    local root = PriorityNode({
		WhileNode(function() return not self.inst.sg:HasStateTag("jumping") end, "AttackAndWander",
			PriorityNode({
				...
				RunAway(self.inst, IsShadowCreature, AVOID_SHADOW_DIST, AVOID_SHADOW_STOP, onrunaway),
