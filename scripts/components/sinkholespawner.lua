...
function SinkholeSpawner:StartSinkholes()
    ...
    for i, v in ipairs(TheNet:GetClientTable() or {}) do
        ...
        if #v.prefab > 0 and (player == nil or GetWorld().Map:IsVisualGroundAtPoint(player.Transform:GetWorldPosition())) then
    ...
    if num_players > 0 then
        ...
        local base_num_attacks = GetWorld().state.remainingdaysinseason <= 2 and 1 or 0
    ...
    elseif targetinfo.warnings >= 0 then
        if targetinfo.pos ~= nil then
            ...
            if ((targetinfo.warnings or 0) % 4 == 0) and targetinfo.player ~= nil and targetinfo.player:IsValid() then
                targetinfo.player.components.talker:Say(GetString(targetinfo.player.prefab, "ANNOUNCE_ANTLION_SINKHOLE"))
            end
        end
...
function SinkholeSpawner:SpawnSinkhole(spawnpt)
    ...
    local function IsValidSinkholePosition(offset)
        ...
        for dx = -1, 1 do
            for dz = -1, 1 do
                if IsPassableAtPoint(x1 + dx * TUNING.ANTLION_SINKHOLE.RADIUS, 0, z1 + dz * TUNING.ANTLION_SINKHOLE.RADIUS) then
                    return false
                end
            end
        end
...
function SinkholeSpawner:PushRemoteTargets()
    ...
    GetWorld():PushEvent("master_sinkholesupdate", { targets = data })
end
    ...
    for i, v in ipairs(self.targets) do
        if v.client ~= nil then
            if v.player ~= nil and v.player:IsValid() then
				if not GetWorld().has_ocean or GetWorld().Map:IsVisualGroundAtPoint(v.player.Transform:GetWorldPosition()) then
