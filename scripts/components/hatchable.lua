...
function Hatchable:GetHeaterPref()
    if GetClock():IsDay() then	
        return self.heater_prefs["day"] 
    elseif GetClock():IsNight() then
        return self.heater_prefs["night"]
    else	
	    return self.heater_prefs["dusk"]
    end
end
...
function Hatchable:OnUpdate(dt)
    ...
    if self.inst.parent then
		...
		if wants_heater ~= nil then
	        if hasfire and not wants_heater then
            --print("   too hot!")
            self.toohot = true
            self:OnState("uncomfy")        
            elseif not hasfire and wants_heater then
            --print("   too cold!")
            self.toocold = true
            self:OnState("uncomfy")
            else
