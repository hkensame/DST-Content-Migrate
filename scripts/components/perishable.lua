...
local function Update(inst, dt)
    if inst.components.perishable then
		if inst.components.perishable.perishremainingtime == nil then
			return
		end
		...
		if owner then
			if owner.components.preserver ~= nil then --改，联机的
			...
			elseif owner:HasTag("foodpreserver") then --改，联机的
                modifier = 0.75 --改，暂时直接赋值TUNING.PERISH_FOOD_PRESERVER_MULT
