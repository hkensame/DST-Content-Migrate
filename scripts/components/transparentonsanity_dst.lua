...
local function PushAlpha(self, alpha, most_alpha)
    self.inst.AnimState:SetAddColour(alpha, alpha, alpha, alpha) --改self.inst.AnimState:OverrideMultColour(alpha, alpha, alpha, alpha)
...
function TransparentOnSanity:OnUpdate(dt)
    ...
    local sanity = player.components.sanity:GetPercent() or 0
    if player == nil then
        self.target_alpha = 0
    elseif self.inst.components.combat and self.inst.components.combat.target then --改if self.inst.components.combat ~= nil then and self.inst.components.combat:GetTarget() == player then
        self.target_alpha = self.most_alpha
    else
        self.offset = self.offset + dt
        self.target_alpha =
            (self.calc_percent_fn and self.calc_percent_fn(self.inst, player) or (1 - sanity)) *  --insanity factor
