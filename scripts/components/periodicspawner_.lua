...
function PeriodicSpawner:TrySpawn(prefab)
    ...
    local inst = nil
    if --[[not GetWorld().Map:IsVisualGroundAtPoint(x,y,z) and--]] not GetPlatformAtPoint(x,z) and GetWorld().components.flotsamgenerator then
        inst = GetWorld().components.flotsamgenerator:SpawnFlotsam(Vector3(x,y,z),prefab,true)        
    else
