...
local RANGED_ATTACK_DSQ = (TUNING.ALTERGUARDIAN_PHASE3_STAB_RANGE or 3.5)^2
local SUMMON_DSQ = (TUNING.ALTERGUARDIAN_PHASE3_SUMMONRSQ or 400) - 36 --364
...
local events =
{
    ...
    EventHandler("doattack", function(inst, data)
        if not (inst.components.health:IsDead() or inst.sg:HasStateTag("busy")) 
            ...
            --GetPlayer().components.talker:Say("距离："..dsq_to_target)
            ...
            if not inst.components.timer:TimerExists("summon_cd") and dsq_to_target < SUMMON_DSQ then
                ...
                local attack_state = "atk_stab" --插入地面？
                ...
                if not inst.components.timer:TimerExists("traps_cd")
                        ...
                        and (geyser_pos == nil
                            or inst:GetDistanceSqToPoint(geyser_pos:Get()) < (TUNING.ALTERGUARDIAN_PHASE3_GOHOMEDSQ / 2)) then --72
                    attack_state = "atk_traps"
                elseif dsq_to_target > RANGED_ATTACK_DSQ then --12.25
...
local STEP = 1.0
local OFFSET = 2 - STEP
local function SpawnBeam(inst, target_pos) --这是发散激光？
    ...
    while i < NUM_STEPS do
        ...
        inst:DoTaskInTime(trigger_time, function(inst2)
            ...
            if first then
                ...
                if player then
                  player.components.playercontroller:ShakeCamera(inst, "FULL", .7, .02, .2, 30)
                end
            end
        end)
...
local function SpawnSweep(inst, target_pos) --这是圆弧激光？
    ...
    while i < num_angle_steps do
        ...
        inst:DoTaskInTime(delay, function(inst2)
            ...
            if first then
                ...
                local player = GetClosestInstWithTag("player", inst, 30)
                if player then
                  player.components.playercontroller:ShakeCamera(inst, "FULL", .7, .02, .6, 30)
                end
            end
        end)
...
local function start_summon_circle(inst) --改，这是什么，那一圈紫色圆？
    local ix, iy, iz = inst.Transform:GetWorldPosition()
...
local function do_summon_spawn(inst) --改，这是大虚影？
    local player_in_range = false
    local ix, _, iz = inst.Transform:GetWorldPosition()
    local p = GetPlayer() --inst.components.combat.target
    --if not p then return end
    --for i, p in ipairs(GetPlayer()) do
        ...
        if dsq_to_player < TUNING.ALTERGUARDIAN_PHASE3_SUMMONRSQ then
            ...
            if (p.components.grogginess_dst ~= nil and not p.components.grogginess_dst:IsKnockedOut()) and not p:HasTag("playerghost") then
...
local function do_stab_attack(inst) --插入地面aoe
...
local states =
{
    State{
        ...
        onenter = function(inst)
            ...
            inst.AnimState:PlayAnimation("phase3_spawn")
    ...
    State{
        ...
        onenter = function(inst)
            ...
            inst.AnimState:SetBank("alterguardian_spawn_death")
            inst.AnimState:PlayAnimation("phase3_death")

            RemovePhysicsColliders(inst)

            set_lightvalues(inst, 0.9)

            GetWorld():PushEvent("moonboss_defeated") --改TheWorld:PushEvent("moonboss_defeated")
        ...
        timeline =
        {
            ...
            TimeEvent(65*FRAMES, function(inst)
                ...
                if player then
                  player.components.playercontroller:ShakeCamera(inst, "FULL", 0.10, 0.05, .1, 40)
                end
            end),
            ...
            TimeEvent(87*FRAMES, function(inst)
                ...
                local player = GetClosestInstWithTag("player", inst, 40)
                if player then
                  player.components.playercontroller:ShakeCamera(inst, "VERTICAL", 0.75, 0.05, 0.6, 40)
                end
            end),
...
CommonStates.AddWalkStates(states,
{
    starttimeline = {
        TimeEvent(3*FRAMES, function(inst)
            inst.SoundEmitter:PlaySound("moonstorm/creatures/boss/alterguardian3/foley")
        end),
    },
    
    walktimeline = {
        TimeEvent(0*FRAMES, function(inst)
            inst.SoundEmitter:PlaySound("moonstorm/creatures/boss/alterguardian3/step")
        end),
