...
local CHOOSE_AOE_RANGE = (TUNING.ALTERGUARDIAN_PHASE1_AOERANGE or 4.25) / 2
...
local function DoAOEAttack(inst, range) --改，震地伤害。蘑菇万岁！
 local pt = Vector3(inst.Transform:GetWorldPosition() )
    local ents = TheSim:FindEntities(pt.x, pt.y, pt.z, range, nil, {"FX", "NOCLICK", "DECOR", "INLIMBO"})
    for i,ent in ipairs(ents) do
      if ent.components.combat
        and ent ~= inst
        and inst.components.combat:IsValidTarget(ent) then
       ent.components.combat:GetAttacked(inst, TUNING.ALTERGUARDIAN_PHASE1_AOEDAMAGE)
       end
    end
end
...
local function tantrum_screenshake(inst)
    ...
    if player then
        player.components.playercontroller:ShakeCamera(inst, "VERTICAL", .5, TANTRUM_SS_SPEED, TANTRUM_SS_SCALE, 60)
    end
end
...
local function roll_screenshake(inst)
    ...
    if player then
        player.components.playercontroller:ShakeCamera(inst, "VERTICAL", .5, ROLL_SS_SPEED, ROLL_SS_SCALE, 40)
    end
end
...
local states =
{
    State{
        ...
        onenter = function(inst)
            ...
            inst.AnimState:PlayAnimation("fall_bounce")
    ...
    State{
        ...
        onenter = function(inst)
            ...
            inst.AnimState:SetBank("alterguardian_spawn_death")
            inst.AnimState:PlayAnimation("phase1_spawn")
            inst.components.health:SetInvincible(true)
...
--改，震地伤害，但是没用，哈哈……
    ...
    State{
        ...
        onenter = function(inst)
            ...
            inst.AnimState:SetBuild("alterguardian_spawn_death")
            --改inst.AnimState:SetBankAndPlayAnimation("alterguardian_spawn_death", "phase1_death")
            inst.AnimState:SetBank("alterguardian_spawn_death")
            inst.AnimState:PlayAnimation("phase1_death")
        ...
        timeline =
        {
            ...
            TimeEvent(26*FRAMES, function(inst)
                ...
                local player = GetClosestInstWithTag("player", inst, 60)
                if player then
                  player.components.playercontroller:ShakeCamera(inst, "FULL", 0.5, 0.1, 0.6, 60)
                end
            end),
        },
