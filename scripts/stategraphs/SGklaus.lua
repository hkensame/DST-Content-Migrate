...
local function ShakeIfClose(inst)
    ...
    if player then
        player.components.playercontroller:ShakeCamera(inst, "FULL", .5, 0.2, .15, 40)
    end
    end
end
...
local function DoRoarShake(inst)
    ...
    local player = GetClosestInstWithTag("player", inst, 30)
    if player then
        player.components.playercontroller:ShakeCamera(inst, "FULL", 1.5, .015, inst.enraged and .3 or .15, 30)
    end
end
...
local function DoChompShake(inst)
    ...
    if player then
        player.components.playercontroller:ShakeCamera(inst, "FULL", 1.5, .015, inst.enraged and .3 or .15, 20)
    end
end
...
local function DoLanding(inst)
    ...
    local player = GetClosestInstWithTag("player", inst, 30)
    if player then
        player.components.playercontroller:ShakeCamera(inst, "FULL", .5, .02, inst.enraged and .2 or .1, 30)
    end
end
...
local states =
{
    ...
    State{
        ...
        timeline =
        {
            TimeEvent(0, function(inst)
                ...
                stafflight:setupfn({ 150 / 255, 46 / 255, 46 / 255 }, 1, 20 * FRAMES)
    ...
    State{
        ...
        onenter = function(inst)
            ...
            for i, v in ipairs(TheSim:FindEntities(x, y, z, 30, nil, TRANSITION_PST_CANT_TAGS)) do
                if v.components.health and not v.components.health:IsDead() and inst.components.grouptargeter:IsTargeting(v.components.combat.target) then
                    StartLaughing(inst)
                    break
                end
            end
...
CommonStates.AddFrozenStates2(states, nil, StopLaughing)
