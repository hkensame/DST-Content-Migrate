...
local CHOP_RANGE = (TUNING.ALTERGUARDIAN_PHASE2_CHOP_RANGE or 4.5)
local SPIN_RANGE = (TUNING.ALTERGUARDIAN_PHASE2_SPIN_RANGE or 18)
local CHOP_RANGE_DSQ = CHOP_RANGE * CHOP_RANGE
local SPIN_RANGE_DSQ = SPIN_RANGE * SPIN_RANGE
local events =
{
...
local states =
{
    State {
        ...
        onenter = function(inst)
            ...
            --改inst.AnimState:SetBankAndPlayAnimation("alterguardian_spawn_death", "phase2_spawn")
            inst.AnimState:SetBank("alterguardian_spawn_death")
            inst.AnimState:PlayAnimation("phase2_spawn")
        ...
        onexit = function(inst)
            ...
            --改inst.AnimState:SetBankAndPlayAnimation("alterguardian_phase2", "idle")
    ...
    State {
        ...
        {   
            ...
            TimeEvent(22*FRAMES, function(inst)
                ...
                if player then
                  player.components.playercontroller:ShakeCamera(inst, "VERTICAL", .75, 0.1, 0.1, 30)
                end
            end),
    ...
    State {
        ...
        timeline =
        {
            ...
            TimeEvent(28*FRAMES, function(inst)
                ...
                local player = GetClosestInstWithTag("player", inst, 50)
                if player then
                  player.components.playercontroller:ShakeCamera(inst, "FULL", .75, 0.1, 0.1, 50)
                end
    ...
    State {
        ...
        onupdate = function(inst)
            ...
            if not inst.sg.statemem.stop_tracking and inst.sg.statemem.target ~= nil and inst.sg.statemem.target:IsValid() then
                ...
                if GetPlatformAtPoint(tpos.x, tpos.z) ~= nil then
        ...
        ontimeout = function(inst)
            ...
            if tpos ~= nil then
                local target_platform = GetPlatformAtPoint(tpos.x, tpos.z)
                if target_platform ~= nil and target_platform.components.walkableplatform:CanBeWalkedOn() then
                    --改，暂时禁用ShakeAllCamerasOnPlatform(CAMERASHAKE.VERTICAL, .75, 0.1, 0.1, target_platform)
    ...
    State {
        ...
        onenter = function(inst)
            ...
            inst.AnimState:SetBank("alterguardian_spawn_death")
            inst.AnimState:PlayAnimation("phase2_death")
        ...
        timeline =
        {
            ...
            TimeEvent(44*FRAMES, function(inst)
                ...
                if player then
                  player.components.playercontroller:ShakeCamera(inst, "FULL", 0.5, 0.1, 0.7, 60)
                end
            end),
