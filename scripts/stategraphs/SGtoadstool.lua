...
local function ShakeIfClose(inst)
    ...
    if player then
        player.components.playercontroller:ShakeCamera(inst, "FULL", .7, 0.2, .5, 40)
    end
    BounceStuff(inst)
end
...
local function DoStompstep(inst)
    ...
    local player = GetClosestInstWithTag("player", inst, 40)
    if player then
        player.components.playercontroller:ShakeCamera(inst, "FULL", .35, 0.2, .7, 40)
    end
    DestroyStuff(inst)
    BounceStuff(inst)
end
...
local function DoRoarShake(inst)
    ...
    local player = GetClosestInstWithTag("player", inst, 40)
    if player then
        player.components.playercontroller:ShakeCamera(inst, "FULL", .7, 0.2, .3, 40)
    end
    BounceStuff(inst)
end
...
local function DoChannelingShake(inst)
    ...
    local player = GetClosestInstWithTag("player", inst, 40)
    if player then
        player.components.playercontroller:ShakeCamera(inst, "VERTICAL", 6 * FRAMES, 0.2, .2, 40)
    end
    BounceStuff(inst)
end
...
local function DoSporeBombShake(inst)
    ...
    local player = GetClosestInstWithTag("player", inst, 40)
    if player then
        player.components.playercontroller:ShakeCamera(inst, "FULL", .35, 0.2, .3, 40)
    end
    BounceStuff(inst)
end
...
local function DoMushroomBombShake(inst)
    ...
    local player = GetClosestInstWithTag("player", inst, 40)
    if player then
        player.components.playercontroller:ShakeCamera(inst, "VERTICAL", .35, 0.2, .3, 40)
    end
    BounceStuff(inst)
end
...
local function DoPoundShake(inst)
    ...
    local player = GetClosestInstWithTag("player", inst, 40)
    if player then
        player.components.playercontroller:ShakeCamera(inst, "VERTICAL", .35, 0.2, .1, 40)
    end
    BounceStuff(inst)
end
...
local states =
{
    ...
    State{
        ...
        timeline =
        {
            TimeEvent(0 * FRAMES, function(inst)
                ...
                inst.SoundEmitter:PlaySound("toadstool/toadstool/DST_toadstool_spawn_appear_pre_" .. math.random(1,3))
            end),
            TimeEvent(10 * FRAMES, function(inst)
                --ShakeAllCameras(CAMERASHAKE.VERTICAL, 40 * FRAMES , .03, 2, inst, 40)
                local player = GetClosestInstWithTag("player", inst, 40)
                if player then
                  player.components.playercontroller:ShakeCamera(inst, "VERTICAL", 40 * FRAMES, .03, 2, 40)
                end
            end),
            TimeEvent(12 * FRAMES, function(inst)
                inst.SoundEmitter:PlaySound("dontstarve/creatures/together/toad_stool/spawn_appear")
                inst.SoundEmitter:PlaySound("toadstool/toadstool/DST_toadstool_spawn_appear_" .. math.random(1,3))
    ...
    State{
        ...
        timeline =
        {
            ...
            TimeEvent(21 * FRAMES, function(inst)
                ...
                local player = GetClosestInstWithTag("player", inst, 40)
                if player then
                  player.components.playercontroller:ShakeCamera(inst, "VERTICAL", 20 * FRAMES, .03, 2, 40)
                end
            end),
            TimeEvent(40 * FRAMES, function(inst)
                ...
                local player = GetClosestInstWithTag("player", inst, 40)
                if player then
                  player.components.playercontroller:ShakeCamera(inst, "VERTICAL", 30 * FRAMES, .03, .7, 40)
                end
            end),
    ...
    State{
        ...
        timeline =
        {
            TimeEvent(8 * FRAMES, function(inst)
                ...
                inst.SoundEmitter:PlaySound("toadstool/toadstool/DST_toadstool_phase_transition_roar_v2_" .. math.random(1,2))
                DoRoarShake(inst)
            end),
    ...
    State{
        ...
        {
            TimeEvent(8 * FRAMES, function(inst)
                GetWorld():PushEvent("ms_miniquake", { rad = 20, num = 20, duration = 2.5, target = inst })
                inst.components.groundpounder:GroundPound()
                BounceStuff(inst)
                inst.SoundEmitter:PlaySound("dontstarve_DLC001/creatures/bearger/groundpound")
            end),
