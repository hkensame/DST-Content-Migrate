		...
		if v ~= inst and
			...
			if v:GetDistanceSqToPoint(x, y, z) < range * range and inst.components.combat:CanTarget(v) then
				...
				if mult ~= nil then
					...
					--[DS移植] knockback 是 DST C++ 引擎特性，DS 无此机制
...
local states =
{
	...
	State{
		...
		onenter = function(inst)
			...
			if inst.SwitchToFacingModel then inst:SwitchToFacingModel(0) end --[DS] DS无此API，nil守卫
		...
		onexit = function(inst)
			if not inst.sg.statemem.tired then
				...
				if inst.sg.statemem.struggling then
					if inst.SwitchToFacingModel then inst:SwitchToFacingModel(6) end --[DS]
				else
					if inst.SwitchToFacingModel then inst:SwitchToFacingModel(4) end --[DS]
				end
			end
		end,
	},
