...
local function FindBestAttackTarget(inst)
    ...
    -- DS 适配：单人模式，直接获取 GetPlayer()
    local v = GetPlayer()
    if v and v.components.health and not v.components.health:IsDead() and not v:HasTag("playerghost") and
...
local function DoSpecialAttack(inst, target)
	...
	local grogginess_dst = target.components.grogginess_dst
	if grogginess_dst ~= nil then 
		grogginess_dst:AddGrogginess(TUNING.GESTALT_ATTACK_DAMAGE_GROGGINESS, TUNING.GESTALT_ATTACK_DAMAGE_KO_TIME)
		if grogginess_dst.knockoutduration == 0 then
