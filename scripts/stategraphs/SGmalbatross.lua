...
local function resetcollision(inst) --改，暂时禁用
...
local function spawnripple(inst)
    --if not  GetWorld().Map:IsVisualGroundAtPoint(inst.Transform:GetWorldPosition()) then --如果不是在陆地上
...
local function spawnsplash(inst, size, pos)
    ...
    --if not  GetWorld().Map:IsVisualGroundAtPoint(splashpos.x,splashpos.y,splashpos.z) and not  GetWorld().Map:GetPlatformAtPoint(splashpos.x,splashpos.z) then
...
local function spawnwave(inst, time)
    --Spawnwaves(inst, 12, 360, 4, nil, 2, time or 2, nil, true)
    SpawnWaves(inst, 12, 360, 4, nil, 2, time or 2, nil, true)
end
...
local function SpawnMalbatrossAttackWaves(inst)
    ...
    local platform =  inst:GetPlatformAtPoint(point.x,point.y,point.z)
    if platform then
    ...
    elseif not  GetWorld().Map:IsVisualGroundAtPoint(point.x,point.y,point.z) then
...
local function AoeAttack(inst)
 local pt = Vector3(inst.Transform:GetWorldPosition() )
    local ents = TheSim:FindEntities(pt.x, pt.y, pt.z, TUNING.MALBATROSS_AOE_RANGE, nil, {"FX", "NOCLICK", "DECOR", "INLIMBO"})
    for i,ent in ipairs(ents) do
      if ent.components.combat
        and ent ~= inst
        and inst.components.combat:IsValidTarget(ent) then
       ent.components.combat:GetAttacked(inst, 150)
GetPlayer().components.talker:Say("阿巴阿巴阿巴")
       end
    end
end
...
local events =
{
    ...
    --CommonHandlers.OnSleepEx(), --改，暂时禁用
    ...
    EventHandler("dosplash", function(inst, data)
        if not inst.components.health:IsDead() and not inst.components.freezable:IsFrozen() and not inst.components.sleeper:IsAsleep() then
            --if not  GetWorld().Map:IsVisualGroundAtPoint(inst.Transform:GetWorldPosition()) and not inst:GetCurrentPlatform() then
            if inst:GetIsOnWater() then
    ...
    EventHandler("death", function(inst, data)
        --if  GetWorld().Map:IsVisualGroundAtPoint(inst.Transform:GetWorldPosition()) or inst:GetCurrentPlatform() then
...
local states =
{
    ...
    State{
        ...
        {
            TimeEvent(5 * FRAMES, function(inst) --inst.SoundEmitter:PlaySound("saltydog/creatures/boss/malbatross/taunt") end),
              inst.SoundEmitter:PlaySound("dontstarve_DLC002/creatures/quacken/taunt") end),
            TimeEvent(8 * FRAMES, spawnripple),
        },
    ...
    State{
        ...
        onexit = function(inst)
            ...
            --inst.components.combat:EnableAreaDamage(true) --俯冲伤害？
            AoeAttack(inst)
        end,
    ...
    State{
        ...
        timeline =
        {
            ...
            TimeEvent(19 * FRAMES, function(inst)
                ...
                spawnsplash(inst)
                spawnwave(inst, 1)
                inst:DoTaskInTime(0.7, function() spawnwave(inst, 1) end)
                inst:DoTaskInTime(1.3, function() spawnwave(inst, 1) end)
                inst.DynamicShadow:Enable(false)
                inst.sg:AddStateTag("noattack")
            end),
        },
...
--]]
        onenter = function(inst)
            if inst.components.combat and inst.components.combat.target then
                ...
                local pt = Vector3(inst.Transform:GetWorldPosition())
                local angle = inst:GetAngleToPoint(Vector3(inst.components.combat.target.Transform:GetWorldPosition())) + 180
                local offset = Vector3(inst.components.combat.target.Transform:GetWorldPosition()) + Vector3(math.random(-5,5),0,math.random(-5,5))
                inst.Transform:SetPosition(offset.x,pt.y,offset.z)
            end
