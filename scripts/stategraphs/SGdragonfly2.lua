...
local function onattackedfn(inst)
    if (not inst.sg:HasStateTag("busy") or inst.sg:HasStateTag("caninterrupt")) and not inst.components.health:IsDead() then
        ...
        elseif (inst.sg.mem.last_hit_time or 0) + 2 <= GetTime() then
...
local function SwitchToFlyOverPhysics(inst)
    if not inst.sg.mem.flyoverphysics then
        ...
        inst.Physics:CollidesWith(192)-- COLLISION.WORLD
...
local states =
{
    ...
    State{
        ...
        onenter = function(inst)
            ...
            inst.components.timer:StartTimer("groundpound_cd", 20)
        end,
...
return StateGraph("dragonfly2", states, events, "idle", actionhandlers)
