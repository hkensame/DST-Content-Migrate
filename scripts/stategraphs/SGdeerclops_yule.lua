...
local function SpawnLaser(inst)
    ...
    while i < numsteps do
        ...
        if not IsPassableAtPoint(x1, 0, z1) then
            if i <= 0 then
                return
            end
            noground = true
    ...
    deathtimeline =
    {
        ...
        TimeEvent(32 * FRAMES, function(inst)
            if TUNING.WINTERS_FEAST then
                ...
                --LaunchAt(SpawnPrefab("winter_ornament_light1"), inst, player, 1, 6, .5) --改
...
return StateGraph("deerclops_yule", states, events, "idle", actionhandlers)
