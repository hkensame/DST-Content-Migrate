...
local function ShakeRaising(inst)
    local player = GetClosestInstWithTag("player", inst, SHAKE_DIST)
    if player then
        player.components.playercontroller:ShakeCamera(inst, "FULL", 1, 0.02, 1, SHAKE_DIST)
    end
end
...
local events =
{
    ...
    EventHandler("attacked", function(inst)
        ...
        if not inst.components.health:IsDead() and
            ...
            (inst.sg.mem.last_hit_time or 0) + TUNING.ANTLION_HIT_RECOVERY < GetTime() or inst.sg:HasStateTag("frozen") then
...
local states =
{
    ...
    State{
        ...
        timeline =
        {
            TimeEvent(0 * FRAMES, function(inst) inst.SoundEmitter:PlaySound("antlion/antlion/death") end),
    ...
    State{
        ...
        timeline =
        {
            ...
            TimeEvent(9 * FRAMES, function(inst) inst.SoundEmitter:PlaySound("antlion/antlion/attack_pre") end),
            TimeEvent(14 * FRAMES, function(inst)
    ...
    State{
        ...
        timeline =
        {
            TimeEvent(5 * FRAMES, function(inst)
                inst.sg:AddStateTag("nosleep")
                inst.sg:AddStateTag("nofreeze")
                inst.SoundEmitter:PlaySound("antlion/antlion/taunt")
            end),
