...
local states =
{
	...
	State{
		...
		timeline =
		{
			FrameEvent(6, function(inst) inst.sg:AddStateTag("noelectrocute") end),
			FrameEvent(11, function(inst) inst.SoundEmitter:PlaySound("daywalker/pillar/chain_shake_oneshot") end),
		},
		events = { EventHandler("animover", function(inst) if inst.AnimState:AnimDone() then inst.sg:GoToState("struggle3_loop_a", true) end end), },
	},
	...
	State{
		...
		events = { EventHandler("animover", function(inst) if inst.AnimState:AnimDone() then inst.sg:GoToState("struggle3_loop_b", inst.sg.statemem.skipsound) end end), },
	},
		...
		timeline =
		{
			...
			FrameEvent(8, function(inst)
				...
				if inst.sg.statemem.skipsound then
					...
					if not all then inst.sg.mem.pendingbreak = nil
					elseif inst.sg.mem.pendingbreak then
						inst.sg.mem.pendingbreak = nil
						inst.sg.statemem.dobreak = true
					else inst.sg.mem.pendingbreak = true end
					if not inst.sg.statemem.dobreak then
						inst.SoundEmitter:PlaySound("daywalker/voice/struggle3")
					end
				end
			end),
			FrameEvent(9, function(inst) if inst.sg.statemem.dobreak then inst.SoundEmitter:PlaySound("daywalker/voice/chainbreak_break_1") end end),
		},
		events = { EventHandler("animover", function(inst)
			if inst.AnimState:AnimDone() then
				if inst.sg.statemem.dobreak then inst.sg:GoToState("chain_break")
				else
					local any, all = CheckPillars(inst)
					inst.sg.mem.pendingbreak = all or nil
					if any then inst.sg:GoToState(math.random() < 0.5 and "struggle3_loop_a" or "struggle3_loop_c", not inst.sg.statemem.skipsound)
					else inst.sg:GoToState("struggle3_loop_c", not inst.sg.statemem.skipsound) end
				end
			end
		end), },
	},
	...
	State{
		...
		events = { EventHandler("animover", function(inst)
			if inst.AnimState:AnimDone() then
				if inst.sg.statemem.skipsound then inst.sg:GoToState("struggle3_loop_b", true)
				else
					local any, all = CheckPillars(inst)
	...
	State{
		...
		timeline =
		{
			...
			FrameEvent(3, function(inst) inst.sg:AddStateTag("noelectrocute") end),
		},
		events = { EventHandler("animover", function(inst) if inst.AnimState:AnimDone() then inst.sg:GoToState("idle") end end), },
	},
	...
	State{
		...
		timeline = { FrameEvent(11, function(inst) inst.SoundEmitter:PlaySound("daywalker/pillar/chain_shake_oneshot") end), },
		events = { EventHandler("animover", function(inst) if inst.AnimState:AnimDone() then inst.sg.statemem.breaking = true; inst.sg:GoToState("chain_break") end end), },
		onexit = function(inst) if not inst.sg.statemem.breaking then inst:RemoveTag("NOCLICK") end end,
	},
	...
	State{
		...
		timeline =
		{
			...
			FrameEvent(71, function(inst) inst:RemoveTag("NOCLICK") end),
		},
		events = { EventHandler("animover", function(inst) if inst.AnimState:AnimDone() then inst:MakeUnchained() end end), },
		onexit = function(inst) inst:RemoveTag("NOCLICK") end,
	},
}
