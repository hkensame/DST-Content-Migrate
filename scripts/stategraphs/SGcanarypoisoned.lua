-- 毒化金丝雀 状态图
-- 移植�?DST，适配 DS 单人生存模式
-- 移除：OnElectrocute、AddElectrocuteStates（DST 独有闪电系统�?
-- 修复：HasAnyStateTag �?多个 HasStateTag �?or 连接
...
local function CheckRecovery(inst)
    ...
    return world ~= nil
        and world.components.birdspawner ~= nil
        and world.components.toadstoolspawner == nil
end
...
local events =
{
    ...
    --CommonHandlers.OnElectrocute(), -- DST 独有，DS 无闪电系�?
    EventHandler("attacked", function(inst, data)
        if not inst.components.health:IsDead() then
            -- DS 没有 HasAnyStateTag，改为多�?HasStateTag �?or 连接
            if not (inst.sg:HasStateTag("noattack") or inst.sg:HasStateTag("nohit") or inst.sg:HasStateTag("electrocute")) then
...
--CommonStates.AddElectrocuteStates(states, nil, {...}) -- DST 独有，DS 无闪电系�?
